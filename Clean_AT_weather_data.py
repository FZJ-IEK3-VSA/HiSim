...
raw_folder = os.path.join(base_dir, "TRY_au_data", "2011-2030", "raw")
cleaned_folder = os.path.join(base_dir, "TRY_au_data", "2011-2030", "cleaned")
...
for file in os.listdir(raw_folder):
    ...
    data_au.index = pd.date_range("2011-01-01 00:30:00", periods=8760, freq="H", tz="UTC")

    for col in ["T", "GHI", "Wspd"]:
        data_au[col] = pd.to_numeric(data_au[col].astype(str).str.replace(",", ".", regex=False), errors="coerce")

    # ── Calculate solar components ──────────────────────────────────────────
    data_au["GHI"] = data_au["GHI"] * 3.0
    ...
    solar_position = pvlib.solarposition.get_solarposition(time=data_au.index, latitude=lat, longitude=lon)

    data_au["DNI"] = pvlib.irradiance.dirint(data_au["GHI"], solar_position["zenith"], data_au.index).clip(lower=0)
