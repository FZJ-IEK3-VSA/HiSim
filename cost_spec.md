...
band invariant min ≤ avg ≤ max forces revenue entries to be *mirrored* (§3.9: "minimum" reads
best-case, not LOW-world), so a slot-coherent transfer entry is unrepresentable. Consequently the
invariant is: exact equality in the AVERAGE slot always; exact per-slot equality for rulesets that
...
- `System cost per unit of heat [EUR/kWh] (operating)` (renamed from `Levelized cost of heat`,
  owner decision Q27 R1; the result field keeps its `levelized_cost_of_heat_in_euro_per_kwh` name)
