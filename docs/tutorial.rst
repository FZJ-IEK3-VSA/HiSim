...
``python
python ../hisim/hisim.py simple_examples.py first_example
``

This command executes `hisim.py` on the setup function `first_example` implemented in the file `simple_examples.py` that is stored in `HiSim\system_setups`. The same file contains another setup function that can be used: `second_example`. The results can be visualized under directory `results` created under the same directory where the script with the setup function is located.
...
-----------------------
The directory `HiSim\system_setups` also contains a modular example in the script `modular_example.py`. The first setup function (`setup_function`) can be executed with the following command:
