...
#Inverter_Ratio_list = [0.5, 0.333, 0.25] #Means: Inverter_power_demand  = Battery capacity multiplied with a factor of the list; Battery Capacity = 4 * electrolyzer_energy_

FuelCellPowerW_list = [100000,100000,100000,100000,100000]
Inverter_Ratio_list = [0.5, 0.5, 0.5, 0.5, 0.5]
...
#    for BatteryCapkWh in BatteryCapkWh_list:
    for Inverter_Ratio in Inverter_Ratio_list:
        ...
        BatteryCapkWh = math.ceil(inverte_power_demand_min / 1000 * 4) #Befehl Ceil Rundet auf; BatteryCKapazität in kWh..INverterleistung in Watt gerechnet; Minimum Inverterleistung = 0,25 der Batteriekapazität --> folglich ist die Batteriekapazität immer 4* der Inverterleistung
