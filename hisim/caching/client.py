...
class CacheClient:
    ...
    def lookup_across(self, component_key: str, key_material: str, locations: "CacheLocations") -> CacheEntry:
        """Look an entry up in the locations in order; a miss lands in the first writable directory.
        ...
        The read walk creates nothing and validates each candidate where it lies, so a read-only
        seed directory, or one that does not exist yet, is read past rather than tripped over.
        Only the miss goes through :meth:`lookup`, in the directory
        :meth:`CacheLocations.write_directory` picks, which is the first that can take a write.

        Args:
            component_key: the filename prefix.
            key_material: the string that identifies the entry's inputs.
            locations: the ordered directories to read, of which the first writable one is written.

        Returns:
            CacheEntry: with ``exists`` True for a validated hit anywhere in the list; a miss
                carries the first writable directory as its write path.

        Raises:
            CacheLocationsError: on a miss when no listed directory is writable or creatable.
        """
        filename = self.entry_filename(component_key, CacheEntryMetadata.hash_of(key_material))
        for directory in locations.directories:
            path = os.path.join(directory, filename)
            if self._validated_local_hit(path):
                return CacheEntry(path=path, exists=True, key_material=key_material)
    ...
    def lookup_producer(
        ...
        written to the first writable one (hisim-epc.22).
        ...
        Returns:
            CacheEntry: where the entry is or will be, and whether a validated one is there. On a hit
                ...
                and writes it through :meth:`CacheEntry.writing` into the first writable directory.
        """
