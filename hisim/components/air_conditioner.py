...
class AirConditioner(cp.Component):
    ...
    def build( self, manufacturer, name, min_operation_time, min_idle_time ):
        ...
        air_conditioner_found = False
        for air_conditioner in air_conditioners_database:
            if air_conditioner["Manufacturer"] == manufacturer and air_conditioner["Model"] == name:
                air_conditioner_found = True
                break

        if air_conditioner_found == False:
...
class AirConditionercontroller(cp.Component):
    ...
    def get_building_default_connections( self ):
        ...
        connections.append( cp.ComponentConnection( AirConditionercontroller.TemperatureMean, building_classname, Building.TemperatureMean ) )
        return connections
