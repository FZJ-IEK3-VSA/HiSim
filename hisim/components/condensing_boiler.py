"""Condensing Boiler Module (Brennwertheizung).

This heating system can use different energy carriers (natural, gas, oil, hydrogen, ...) for combustion.
A condensing boiler uses heat not only from the combustion process but also from waste gases
which is why it has higher efficiencies than conventional boilers.
https://www.vaillant.co.uk/advice/understanding-heating-technology/boilers/what-is-a-condensing-boiler/."""
...
@dataclass_json
@dataclass
class CondensingBoilerConfig(ConfigBase):
    """Configuration of the CondensingBoiler class."""

    @classmethod
    def get_main_classname(cls):
        """Return the full class name of the base class."""
        return CondensingBoiler.get_full_classname()
    ...
    maximal_mass_flow_in_kilogram_per_second: float
    ...
    @classmethod
    def get_default_condensing_boiler_config(
        cls,
        building_name: str = "BUI1",
    ) -> Any:
        """Get a default building_name."""
        maximal_power_in_watt: float = 12_000  # W
        config = CondensingBoilerConfig(
    ...
    @classmethod
    def get_scaled_condensing_boiler_config(
        cls,
        heating_load_of_building_in_watt: float,
        building_name: str = "BUI1",
    ) -> "CondensingBoilerConfig":
        """Get a default building_name."""
...
class CondensingBoiler(Component):
    ...
    # Input
    ControlSignal = "ControlSignal"  # at which Procentage is the CondensingBoiler modulating [0..1]
    ...
    def __init__(
        ...
        self.condensing_boiler_config = config
        ...
        self.control_signal_channel: ComponentInput = self.add_input(
            self.component_name,
            CondensingBoiler.ControlSignal,
            lt.LoadTypes.ANY,
            lt.Units.PERCENT,
            True,
        )
        self.mass_flow_input_tempertaure_channel: ComponentInput = self.add_input(
            self.component_name,
            CondensingBoiler.MassflowInputTemperature,
            lt.LoadTypes.WATER,
            lt.Units.CELSIUS,
            True,
        )
        ...
        self.mass_flow_output_channel: ComponentOutput = self.add_output(
            self.component_name,
            CondensingBoiler.MassflowOutput,
            lt.LoadTypes.WATER,
            lt.Units.KG_PER_SEC,
        ...
        self.mass_flow_output_temperature_channel: ComponentOutput = self.add_output(
            self.component_name,
            CondensingBoiler.MassflowOutputTemperature,
            lt.LoadTypes.WATER,
            lt.Units.CELSIUS,
        ...
        self.minimal_thermal_power_in_watt = self.condensing_boiler_config.minimal_thermal_power_in_watt
        self.maximal_thermal_power_in_watt = self.condensing_boiler_config.maximal_power_in_watt
        self.eff_th_min = self.condensing_boiler_config.eff_th_min
        self.eff_th_max = self.condensing_boiler_config.eff_th_max
        self.maximal_temperature_in_celsius = self.condensing_boiler_config.maximal_temperature_in_celsius
        self.temperature_delta_in_celsius = self.condensing_boiler_config.temperature_delta_in_celsius

        self.add_default_connections(self.get_default_connections_from_controller_l1_condensing_boiler())
        self.add_default_connections(self.get_default_connections_from_simple_hot_water_storage())

    def get_default_connections_from_controller_l1_condensing_boiler(
        self,
    ):
        """Get Controller L1 Condensing Boiler default connections."""
        component_class = CondensingBoilerControllerL1
        connections = []
        ...
        connections.append(
            ComponentConnection(
                ...
                component_class.ControlSignalToCondensingBoiler,
            )
        )
        return connections
    ...
    def write_to_report(self) -> List[str]:
        """Write a report."""
        return self.condensing_boiler_config.get_string_dict()
    ...
    def i_simulate(self, timestep: int, stsv: SingleTimeStepValues, force_convergence: bool) -> None:
        """Simulate the Condensing Boiler."""
    ...
    @staticmethod
    def get_cost_capex(config: CondensingBoilerConfig, simulation_parameters: SimulationParameters) -> CapexCostDataClass:
...
@dataclass_json
@dataclass
class CondensingBoilerControllerL1Config(ConfigBase):
    """Gas-heater Controller Config Class."""

    @classmethod
    def get_main_classname(cls):
        """Returns the full class name of the base class."""
        return CondensingBoilerControllerL1.get_full_classname()
    ...
    @classmethod
    def get_default_condensing_boiler_controller_config(
        cls,
        maximal_thermal_power_in_watt: float,
        minimal_thermal_power_in_watt: float = 1000,
        building_name: str = "BUI1",
    ) -> "CondensingBoilerControllerL1Config":
        """Gets a default  Condensing Boiler Controller."""
        return CondensingBoilerControllerL1Config(
            building_name=building_name,
            name="CondensingBoilerController",
            set_heating_threshold_outside_temperature_in_celsius=16.0,
            # get min and max thermal power from Condensing Boiler config
...
class CondensingBoilerControllerL1(Component):
    """Condensing Boiler Controller.

    It takes data from other
    components and sends signal to the condensing_boiler for
    ...
    (1) condensing_boiler (control_signal)
    ...
    # Outputs
    ControlSignalToCondensingBoiler = "ControlSignalToCondensingBoiler"

    def __init__(
        self,
        my_simulation_parameters: SimulationParameters,
        config: GenericCondensingBoilerControllerL1Config,
        ...
        self.control_signal_to_condensing_boiler_channel: ComponentOutput = self.add_output(
            self.component_name,
            self.ControlSignalToCondensingBoiler,
            lt.LoadTypes.ANY,
            lt.Units.PERCENT,
            output_description=f"here a description for {self.ControlSignalToCondensingBoiler} will follow.",
        )

        self.controller_condensing_boilermode: Any
        self.previous_condensing_boiler_mode: Any
    ...
    def get_default_connections_from_simple_hot_water_storage(
        ...
        connections.append(
            ComponentConnection(
                CondensingBoilerControllerL1.WaterTemperatureInputFromHeatWaterStorage,
                storage_classname,
    ...
    def get_default_connections_from_weather(
        ...
        connections.append(
            ComponentConnection(
                CondensingBoilerControllerL1.DailyAverageOutsideTemperature,
                weather_classname,
    ...
    def get_default_connections_from_heat_distribution_controller(
        ...
        connections.append(
            ComponentConnection(
                CondensingBoilerControllerL1.HeatingFlowTemperatureFromHeatDistributionSystem,
    ...
    def build(
        ...
        """
        # Sth
        self.controller_condensing_boilermode = "off"
        self.previous_condensing_boiler_mode = self.controller_condensing_boilermode
    ...
    def i_restore_state(self) -> None:
        """Restore the previous state."""
        self.controller_condensing_boilermode = self.previous_condensing_boiler_mode
    ...
    def i_simulate(self, timestep: int, stsv: SingleTimeStepValues, force_convergence: bool) -> None:
        """Simulate the Condensing Boiler comtroller."""
        ...
        if force_convergence:
            ...
            if self.controller_condensing_boilermode == "heating":
                control_signal = self.modulate_power(
                    water_temperature_input_in_celsius=water_temperature_input_from_heat_water_storage_in_celsius,
                    set_heating_flow_temperature_in_celsius=heating_flow_temperature_from_heat_distribution_system,
                )
            elif self.controller_condensing_boilermode == "off":
                control_signal = 0
            else:
                raise ValueError("Condensing Boiler Controller control_signal unknown.")

            stsv.set_output_value(self.control_signal_to_condensing_boiler_channel, control_signal)
    ...
    def modulate_power(
        ...
        """Modulate linear between minimial_thermal_power and max_thermal_power of Condensing Boiler.

        only used if condensing_boilermode is "heating".
        """
        ...
        raise ValueError("modulation of Condensing Boiler needs some adjustments")
    ...
    def conditions_on_off(
        ...
        """Set conditions for the Condensing Boiler controller mode."""
        ...
        if self.controller_condensing_boilermode == "heating":
            ...
            # Condensing Boiler is only turned on if the water temperature is below the flow temperature
            ...
            if (
                ...
                self.controller_condensing_boilermode = "heating"
                return
    ...
    @staticmethod
    def get_cost_capex(config: CondensingBoilerControllerL1Config, simulation_parameters: SimulationParameters) -> CapexCostDataClass:  # pylint: disable=unused-argument
        """Returns investment cost, CO2 emissions and lifetime."""
