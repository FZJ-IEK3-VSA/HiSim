...
class L1CHPControllerConfig(ConfigBase):
    ...
    def get_default_config_fuel_cell() -> "L1CHPControllerConfig":
        ...
        config = L1CHPControllerConfig(
            ...
            min_idle_time_in_seconds=3600 * 2, forcedrunning = True, on_off_SOEC = 0, off_on_SOEC= 0)
        return config
...
class L1CHPController(cp.Component):
    ...
    def __init__(
        ...
        self.building_temperature_channel: cp.ComponentInput = self.add_input(
            ...
            Units.CELSIUS,
            mandatory=False, #Christof: Change from True to False
        )
    ...
    def calculate_state(self, timestep: int, t_building: float, t_dhw: float,
        ...
        if not hydrogen_soc_ok: #First: Looking if there is enough fuel in the storage
        ...
        if (
            ...
            print("!!!!Colum 426 in controller_l1_chp_CB.py--> there was a minimum chp resting time given but function is not working!!!!")
