...
class ElectrolyzerController(Component):
    ...
    def i_simulate(
        ...
        if current_power >= max_load:
            ...
            print("self.curtailed_load_count: ", self.curtailed_load_count)
        ...
        if current_power >= min_load and self.initial_state == -1: # -1 equals to "off"
            ...
            self.cold_up_count = 0.0
        elif current_power >= min_load and self.initial_state == 0: # 0 equals to "standby"
            self.current_mode = 0.5 # warm starting
            self.initial_state = 111
            self.warm_up_count = 0.0

        if self.current_mode == -0.5 and self.cold_up_count < cold_up_time:
            self.cold_up_count += self.my_simulation_parameters.seconds_per_timestep
            current_power = 0
        ...
        if current_power >= min_load:
            
            if self.current_mode == 1:
                self.current_mode = 1
                stsv.set_output_value(self.distributed_load, current_power)

        standby_threshold = 0.05 * nominal_load
        ...
        if current_power < min_load and current_power >= standby_threshold:
            ...
            elif self.current_mode == 0:  # Check if the system is in the "standby" state
                self.current_mode = -1
                self.initial_state = -1
                self.off_count += 1
                current_power = 0
