...
class L1GenericRuntimeController(cp.Component):
    ...
    def __init__(
        ...
        self.l1_device_signal_channel: cp.ComponentOutput = self.add_output(
            ...
            output_description="On/off signal from the L1 controller commanding the device to run, respecting minimum runtime and resting times.",
        )
        self.l1_runtime_signal: cp.ComponentOutput = self.add_output(
            self.component_name,
            self.L1RunTimeSignal,
            LoadTypes.ANY,
            Units.ANY,
            # This description was generated by Claude Code.
            output_description="Remaining minimum runtime or resting time of the device, in timesteps.",
        )
