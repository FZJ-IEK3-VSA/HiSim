...
class L2GenericEnergyManagementSystem(dynamic_component.DynamicComponent):
    ...
    def __init__(self, my_simulation_parameters: SimulationParameters, config: EMSConfig):
        ...
        self.simple_hot_water_storage_temperature_offset_value = self.ems_config.simple_hot_water_storage_temperature_offset_value
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        self.state.consumption_uncontrolled = sum([stsv.get_input_value(component_input=elem) for elem in self.consumption_uncontrolled_inputs])
        self.state.consumption_ems_controlled = sum([stsv.get_input_value(component_input=elem) for elem in self.consumption_ems_controlled_inputs])
        ...
        electricity_to_grid = self.state.production - self.state.consumption_uncontrolled - self.state.consumption_ems_controlled
