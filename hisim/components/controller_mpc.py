...
class MpcController(cp.Component):
    ...
    def __init__(
        ...
        if self.mpcconfig.prediction_horizon is not None:
            self.prediction_horizon = int(self.mpcconfig.prediction_horizon / self.my_simulation_parameters.seconds_per_timestep)
    ...
    def i_prepare_simulation(self) -> None:
        ...
        if self.mpcconfig.predictive:
            ...
            self.temp_forecast = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.WEATHERTEMPERATUREOUTSIDEYEARLYFORECAST)[
                : self.my_simulation_parameters.timesteps
            ]
            self.phi_m_forecast = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.HEATFLUXTHERMALMASSNODEFORECAST)
            self.phi_st_forecast = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.HEATFLUXSURFACENODEFORECAST)
            self.phi_ia_forecast = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.HEATFLUXINDOORAIRNODEFORECAST)

            """"getting pv forecast"""
            self.pv_forecast_yearly = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.PVFORECASTYEARLY)

            """ getting battery specifications """
            self.maximum_storage_capacity = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.MAXIMUMBATTERYCAPACITY)
            self.minimum_storage_capacity = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.MINIMUMBATTERYCAPACITY)
            self.maximum_charging_power = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.MAXIMALCHARGINGPOWER)
            self.maximum_discharging_power = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.MAXIMALDISCHARGINGPOWER)
    ...
    def statespace(self):
        ...
        state_space_matrix_coefficient_x = ((self.h_tr_w + self.h_tr_ms) * (self.h_ve_adj + self.h_tr_is)) + (self.h_ve_adj * self.h_tr_is)

        # Entries for system matrix
        a11 = (((self.h_tr_ms**2) * (self.h_tr_is + self.h_ve_adj) / state_space_matrix_coefficient_x) - self.h_tr_ms - self.h_tr_em) / (
        ...
        b_d11 = ((self.h_tr_ms * self.h_tr_w * (self.h_tr_is + self.h_ve_adj) / state_space_matrix_coefficient_x) + self.h_tr_em) / (
            self.c_m / seconds_per_timestep
        )
        b_d12 = (self.h_tr_ms * self.h_tr_is * self.h_ve_adj) / ((self.c_m / seconds_per_timestep) * state_space_matrix_coefficient_x)
        b_d13 = (self.h_tr_ms * self.h_tr_is) / ((self.c_m / seconds_per_timestep) * state_space_matrix_coefficient_x)
        b_d14 = (self.h_tr_ms * (self.h_tr_is + self.h_ve_adj)) / ((self.c_m / seconds_per_timestep) * state_space_matrix_coefficient_x)
    ...
    def get_forecast_24h(self, start_horizon, sampling_rate):
        ...
        self.price_purchase_forecast_24h_1min = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.PRICEPURCHASEFORECAST24H)
        self.price_injection_forecast_24h_1min = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.PRICEINJECTIONFORECAST24H)
    ...
    def optimize(  # noqa: C901
        ...
        cop_values = opti.parameter(1, scaled_horizon)  # coefiiecient of performance: heating air conditioner efficiency
        ...
        for k in range(scaled_horizon):
            ...
            if self.flexibility_element == "PV_and_Battery":
                opti.subject_to(
                    ...
                    + (optvar_battery_power_flow[:, k]) * (self.my_simulation_parameters.seconds_per_timestep * sampling_rate / 3600)
                )
        ...
        opti.subject_to(optvar_disturbances == disturbance_forecast)  # building disturbances (solar gains / internal gains / ambient temperature)

        if self.flexibility_element in {"PV_only", "PV_and_Battery"}:
            opti.subject_to(optvar_power_pv_generation_forecasted == pv_production)  # forecasted PV generation by the generic_pv_component
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:  # noqa: C901
        ...
        if self.mpcconfig.predictive:
            if (self.mpc_scheme == "optimization_once_aday_only" and timestep % self.prediction_horizon == 0) or (
                self.mpc_scheme == "moving_horizon_control" and timestep <= self.my_simulation_parameters.timesteps - self.prediction_horizon
            ):
