...
class Car(cp.Component):
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if self.config.fuel == lt.LoadTypes.ELECTRICITY:
            watt_used = (
                self.meters_driven[timestep] * self.config.consumption_per_km * (3600 / self.my_simulation_parameters.seconds_per_timestep)
            )  # conversion Wh to W
        ...
        # if not already running: check if activation makes sense
            ...
            liters_used = self.meters_driven[timestep] * self.config.consumption_per_km * 1e-3  # conversion meter to kilometer
    ...
    def build(self, config: CarConfig, occupancy_config: Any) -> None:
        ...
        if file_exists:
            ...
            seconds_per_timestep_original = (
                time_resolution_original.hour * 3600 + time_resolution_original.minute * 60 + time_resolution_original.second
            )
            minutes_per_timestep = int(self.my_simulation_parameters.seconds_per_timestep / seconds_per_timestep_original)
            ...
            initial_data = pd.DataFrame(
                {
                    ...
                    "car_location": [location_translator[elem] for elem in car_location["Values"]][:steps_desired_in_minutes],
                }
            )
            ...
            if minutes_per_timestep > 1:
                for i in range(steps_desired):
                    self.meters_driven.append(sum(meters_driven[i * minutes_per_timestep : (i + 1) * minutes_per_timestep]))  # sum
                    location_list = car_location[i * minutes_per_timestep : (i + 1) * minutes_per_timestep]  # extract list
