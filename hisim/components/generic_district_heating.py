...
class DistrictHeatingConfig(ConfigBase):
    ...
    @classmethod
    def get_default_district_heating_config(cls, building_name: str = "BUI1", with_domestic_hot_water_preparation=False) -> Any:
        """Get a default district heating."""
        config = DistrictHeatingConfig(
            building_name=building_name,
            name="DistrictHeating",
            connected_load_w=20000,
            co2_footprint=4.780735, # source: https://www.oekobaudat.de/OEKOBAU.DAT/datasetdetail/process.xhtml?lang=de&uuid=dcd5e23a-9bec-40b6-b07c-1642fe696a2e Production and transport
...
class DistrictHeating(Component):
    ...
    def __init__(
        ...
        self.heating_mode_channel: ComponentInput = self.add_input(
            self.component_name, DistrictHeating.HeatingMode, LoadTypes.ANY, Units.ANY, True
        )
        self.delta_temperature_channel: ComponentInput = self.add_input(
            self.component_name, DistrictHeating.DeltaTemperatureNeeded, LoadTypes.TEMPERATURE, Units.CELSIUS, True,
        )
        self.water_input_temperature_sh_channel: ComponentInput = self.add_input(
            self.component_name, DistrictHeating.WaterInputTemperatureSh, LoadTypes.WATER, Units.CELSIUS, True,
        )
    ...
    def get_default_connections_from_district_heating_controller(self,):
        ...
        connections.append(
            ComponentConnection(
                DistrictHeating.HeatingMode, controller_classname, component_class.HeatingMode,
            )
        )
        connections.append(
            ComponentConnection(
                DistrictHeating.DeltaTemperatureNeeded, controller_classname, component_class.DeltaTemperatureNeeded,
            )
        )
        return connections
    ...
    def get_default_connections_from_heat_distribution_system(self,):
        ...
        connections.append(
            ComponentConnection(
                DistrictHeating.WaterInputTemperatureSh, hws_classname, component_class.WaterTemperatureOutput,
            )
        )
        connections.append(
            ComponentConnection(
    ...
    def i_simulate(self, timestep: int, stsv: SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if heating_mode == HeatingMode.SPACE_HEATING:
            ...
            thermal_power_delivered_w = water_mass_flow_rate_in_kg_per_s * PhysicsConfig.get_properties_for_energy_carrier(
                energy_carrier=LoadTypes.WATER).specific_heat_capacity_in_joule_per_kg_per_kelvin * delta_temperature_needed_in_celsius
            ...
            if thermal_power_delivered_w > self.config.connected_load_w:
                ...
                delta_temperature_achieved = thermal_power_delivered_w / (water_mass_flow_rate_in_kg_per_s * PhysicsConfig.get_properties_for_energy_carrier(
                energy_carrier=LoadTypes.WATER).specific_heat_capacity_in_joule_per_kg_per_kelvin)
            ...
            stsv.set_output_value(self.thermal_output_energy_sh_channel, thermal_energy_delivered_in_watt_hour)
            stsv.set_output_value(self.water_output_temperature_sh_channel, water_output_temperature_deg_c)
            stsv.set_output_value(self.water_mass_flow_sh_output_channel, water_mass_flow_rate_in_kg_per_s)
            ...
            stsv.set_output_value(self.water_output_temperature_dhw_channel, current_dhw_water_temperature_deg_c)
            ...
            thermal_power_delivered_w = self.config.connected_load_w if delta_temperature_needed_in_celsius > 0 else 0
            ...
            # Set outputs
            stsv.set_output_value(self.thermal_output_power_dhw_channel, thermal_power_delivered_w)
            stsv.set_output_value(self.thermal_output_energy_dhw_channel, thermal_energy_delivered_in_watt_hour)
            stsv.set_output_value(self.water_output_temperature_dhw_channel, water_target_temperature_deg_c)
            stsv.set_output_value(self.water_mass_flow_dhw_output_channel, water_mass_flow_rate_in_kg_per_s) 
            ...
            stsv.set_output_value(self.water_output_temperature_sh_channel, current_sh_water_temperature_deg_c)
...
class DistrictHeatingControllerConfig(ConfigBase):
    ...
    def get_default_district_heating_controller_config(
        cls, building_name: str = "BUI1", with_domestic_hot_water_preparation=False
    ) -> Any:
...
class DistrictHeatingController(Component):
    ...
    def i_simulate(self, timestep: int, stsv: SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if force_convergence:
            ...
            self.determine_operating_mode(
                ...
                water_temperature_input_dhw_in_celsius=water_temperature_input_from_warm_water_storage_in_celsius if self.config.with_domestic_hot_water_preparation else None,
                set_heating_flow_temperature_dhw_in_celsius=self.warm_water_temperature_aim_in_celsius if self.config.with_domestic_hot_water_preparation else None,
    ...
    def determine_operating_mode(
        ...
        def dhw_heating_needed(controller_mode, current_water_temperature, target_water_temperature):
            ...
            if current_water_temperature < target_water_temperature:
                return True
            elif controller_mode == HeatingMode.DOMESTIC_HOT_WATER and current_water_temperature < target_water_temperature + self.config.offset:
                return True
            else:
                return False
        ...
        needs_space_heating = space_heating_needed(water_temperature_input_sh_in_celsius, set_heating_flow_temperature_sh_in_celsius)
        needs_dhw_heating = dhw_heating_needed(self.controller_mode, water_temperature_input_dhw_in_celsius, set_heating_flow_temperature_dhw_in_celsius)
