...
class GenericElectrolyzerConfig:
    ...
    def __init__(self, name: str, source_weight: int, min_power: float, max_power: float, min_hydrogen_production_rate_hour: float,
    ...
    def get_default_config() -> Any:
        config = GenericElectrolyzerConfig(name="Electrolyzer", source_weight=1, min_power=1200,  # [W]
                                           max_power=2400,  # [W]
...
class GenericElectrolyzer(cp.Component):
    ...
    def __init__(self, my_simulation_parameters: SimulationParameters, config: GenericElectrolyzerConfig):
        ...
        self.ElectricityTargetC: cp.ComponentInput = self.add_input(self.component_name, GenericElectrolyzer.ElectricityTarget,
                                                                    lt.LoadTypes.ELECTRICITY, lt.Units.WATT, True)
        self.HydrogenOutputC: cp.ComponentOutput = self.add_output(self.component_name, GenericElectrolyzer.HydrogenOutput, lt.LoadTypes.HYDROGEN,
                                                                   lt.Units.KG_PER_SEC)
    ...
    def get_default_connections_from_L1GenericElectrolyzerController(self) -> List[cp.ComponentConnection]:
        ...
        connections.append(
            cp.ComponentConnection(GenericElectrolyzer.ElectricityTarget, controller_classname, L1GenericElectrolyzerController.ElectricityTarget))
        return connections
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if electricity_target == 0:
            ...
            hydrogen_output_liter = self.min_hydrogen_production_rate + (
                        (self.max_hydrogen_production_rate - self.min_hydrogen_production_rate) * (self.state.electricity - self.min_power) / (
...
class L1ElectrolyzerConfig:
    ...
    def __init__(self, name: str, source_weight: int, min_operation_time: int, min_idle_time: int, P_min_electrolyzer: float,
    ...
    @staticmethod
    def get_default_config() -> Any:
        config = L1ElectrolyzerConfig(name='L1ElectrolyzerRuntimeController', source_weight=1, min_operation_time=14400, min_idle_time=7200,
                                      P_min_electrolyzer=1200, SOC_max_H2=96)
        return config
...
class L1ElectrolyzerControllerState:
    ...
    def clone(self) -> Any:
        return L1ElectrolyzerControllerState(timestep_actual=self.timestep_actual, state=self.state,
...
class L1GenericElectrolyzerController(cp.Component):
    ...
    def __init__(self, my_simulation_parameters: SimulationParameters, config: L1ElectrolyzerConfig) -> None:
        ...
        # add inputs
        self.l2_ElectricityTargetC: cp.ComponentInput = self.add_input(self.component_name, self.l2_ElectricityTarget, lt.LoadTypes.ELECTRICITY,
    ...
    def get_default_connections_from_hydrogenstorage(self) -> List[cp.ComponentConnection]:
        ...
        connections.append(cp.ComponentConnection(L1GenericElectrolyzerController.HydrogenSOC, h2storage_classname,
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if (self.state0.state == 1 and self.state0.timestep_of_last_action + self.on_time >= timestep):
            self.state.state = 1
            l2_electricity_target = max(self.Pmin,
                                        l2_electricity_target)  # return device off if minimum idle time is not fulfilled and device was off in previous state
