...
class Electrolyzer(cp.Component):
    ...
    @staticmethod
    def spec_el_stack_demand_and_polarization_data_config(electrolyzer_type, nominal_load, h2_flow_rate, faraday_eff, i_cell_nom):
        ...
        if electrolyzer_type not in data:
            raise ValueError(f"{electrolyzer_type} is invalid technology. Supported technologies are: {', '.join(data.keys())}")
        ...
        spec_el_stack_demand_nom = (np.array(u_cell) * (2 * f_constant) / (m_h2 / 1000) / 1000 / 3600) / faraday_eff  # kWh/kg
