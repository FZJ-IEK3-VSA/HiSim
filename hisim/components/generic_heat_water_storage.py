...
class HeatStorage(Component):
    ...
    def calculate_new_storage_temperature(
        ...
        temperature_sp = (
            temperature_sp
            + (1 / (mass_sp_h * c_w)) * (production - last - heat_loss_storage * (temperature_sp - temperature_ext_sp)) * seconds_per_timestep
        )
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if stsv.get_input_value(self.control_signal_choose_storage_channel) == 1:  # choose to heat up warm water storage
        ...
        elif stsv.get_input_value(self.control_signal_choose_storage_channel) == 2:  # choose to heat up heating water storage
            production_var = self.adding_all_possible_mass_flows(stsv)
            result_hw = self.calculate_new_storage_temperature(
                seconds_per_timestep=self.my_simulation_parameters.seconds_per_timestep,
                temperature_sp=temperature_sp_var_hw,
...
class HeatStorageController(cp.Component):
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        max_last_var_hw = (
            max_mass_flow_heat_storage * 4.185 * 1000 * (temperature_sp_var_hw - stsv.get_input_value(self.building_temperature_channel))
        )
