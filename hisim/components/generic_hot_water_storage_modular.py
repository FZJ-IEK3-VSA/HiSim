...
class StorageConfig:
    ...
    def get_default_config_boiler():
        ...
        config = StorageConfig(
            ...
            surface=2.0,
            u_value=0.36,
            warm_water_temperature=50,
            drain_water_temperature=10,
            efficiency=1,
            power=None,
...
class StorageState:
    ...
    def return_available_energy(self, heating: bool) -> float:
        ...
        if heating:
            ...
            available_energy = (
                (self.temperature_in_kelvin - 273.15 - 21)
                * self.volume_in_l
                * 0.977
                * 4.182
                * 1e3
...
class HotWaterStorage(dycp.DynamicComponent):
    ...
    def build(self, config: StorageConfig) -> None:
        ...
        if self.cooling_considered:
            if isinstance(config.heating_season_begin, int) and isinstance(
                config.heating_season_end, int
            ):
    ...
    def calculate_heat_consumption(
        ...
        if self.use == lt.ComponentType.BOILER:
            ...
            return (
                stsv.get_input_value(self.water_consumption_c)
        ...
        if self.use == lt.ComponentType.BUFFER:
            ...
            heatconsumption = (
                stsv.get_input_value(self.l1_device_signal_c)
                * self.power
                * self.my_simulation_parameters.seconds_per_timestep
                * 1e-3
            )  # 1e-3 conversion J to kJ
            if self.cooling_considered:
                if self.heating_season_end < timestep < self.heating_season_begin:
                    available_energy = (
                        self.state.return_available_energy(heating=False)
                        + thermal_energy_delivered
                    )
                ...
                available_energy = (
                    self.state.return_available_energy(heating=True)
                    + thermal_energy_delivered
                )
                if heatconsumption > available_energy:
