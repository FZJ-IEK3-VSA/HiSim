...
class HotWaterStorage(cp.Component):
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if self.thermal_power_delivered_channel.source_output is not None:
            thermal_energy_delivered = (
                thermal_energy_delivered
                + stsv.get_input_value(self.thermal_power_delivered_channel) * self.my_simulation_parameters.seconds_per_timestep * 1e-3
            )  # 1e-3 conversion J to kJ
        if self.thermal_power_chp_channel.source_output is not None:
            thermal_energy_delivered = (
                thermal_energy_delivered
                + stsv.get_input_value(self.thermal_power_chp_channel) * self.my_simulation_parameters.seconds_per_timestep * 1e-3
            )  # 1e-3 conversion J to kJ
        ...
        new_energy = (
            energy
            - (self.state.temperature_in_kelvin - 293) * self.surface * self.u_value * self.my_simulation_parameters.seconds_per_timestep * 1e-3
            - heatconsumption
            + thermal_energy_delivered
        )
    ...
    def calculate_heat_consumption(
        ...
        if self.use == lt.ComponentType.BOILER:
            ...
            return stsv.get_input_value(self.water_consumption_channel) * (self.warm_water_temperature - self.drain_water_temperature) * 0.977 * 4.182
        if self.use == lt.ComponentType.BUFFER:
            heatconsumption = (
