...
class GenericHydrogenStorage(cp.Component):
    ...
    def store(self, charging_rate: float) -> Tuple[float, float, float]:
        ...
        if self.state.fill + charging_rate * self.my_simulation_parameters.seconds_per_timestep < self.config.max_capacity:
            # fits completely
    ...
    def withdraw(self, discharging_rate: float) -> Tuple[float, float, float]:
        ...
        if self.state.fill > self.config.min_capacity + discharging_rate * self.my_simulation_parameters.seconds_per_timestep:
            # has enough
