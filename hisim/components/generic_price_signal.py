...
class PriceSignalConfig(ConfigBase):
    ...
    country: str
    pricing_scheme: str
    installed_capacity_in_kW: float
    price_signal_type: str
    fixed_price_in_cent_per_kW_timestep: List[float]
    static_tou_price_in_cent_per_kW_timestep: List[float]
    price_injection_in_cent_per_kW_timestep: float
    ...
    def get_default_price_signal_config(
        ...
        config = PriceSignalConfig(
            ...
            installed_capacity_in_kW=10e3,
            price_signal_type="dummy",
            fixed_price_in_cent_per_kW_timestep=[],
            static_tou_price_in_cent_per_kW_timestep=[],
            price_injection_in_cent_per_kW_timestep=0.0,
...
class PriceSignal(cp.Component):
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        """
        price_injection_forecast_in_cent_per_kW_timestep = [0.1]
        price_purchase_forecast_in_cent_per_kW_timestep = [0.5]
        if self.config.predictive_control and self.config.prediction_horizon_in_s:
            price_injection_forecast_in_cent_per_kW_timestep = [0.1] * int(
                self.config.prediction_horizon_in_s / self.my_simulation_parameters.seconds_per_timestep
            )
            price_purchase_forecast_in_cent_per_kW_timestep = [0.5] * int(
                self.config.prediction_horizon_in_s / self.my_simulation_parameters.seconds_per_timestep
            )
        elif self.price_signal_config.price_signal_type == "Prices at second half of 2021":
            price_injection_forecast_in_cent_per_kW_timestep = [self.price_signal_config.price_injection_in_cent_per_kW_timestep] * int(
                self.config.prediction_horizon_in_s / self.my_simulation_parameters.seconds_per_timestep
            )
        elif self.price_signal_config.pricing_scheme == "dynamic":
            price_purchase_forecast_in_cent_per_kW_timestep = self.price_signal_config.static_tou_price_in_cent_per_kW_timestep
        elif self.price_signal_config.pricing_scheme == "fixed":
            price_purchase_forecast_in_cent_per_kW_timestep = self.price_signal_config.fixed_price_in_cent_per_kW_timestep
        elif self.price_signal_config.price_signal_type == "dummy":
            price_injection_forecast_in_cent_per_kW_timestep = [10] * int(
                self.config.prediction_horizon_in_s / self.my_simulation_parameters.seconds_per_timestep
            )
            price_purchase_forecast_in_cent_per_kW_timestep = [50] * int(
                self.config.prediction_horizon_in_s / self.my_simulation_parameters.seconds_per_timestep
            )
            # price_purchase_forecast_in_cent_per_kW_timestep = [ ]
            # for step in range( self.day ):
            #     x = timestep % self.day
            #     if x > self.start and x < self.end:
            #         price_purchase_forecast_in_cent_per_kW_timestep.append( 20 * self.my_simulation_parameters.seconds_per_timestep / 3.6e6 )
            #     else:
            #         price_purchase_forecast_in_cent_per_kW_timestep.append( 50 * self.my_simulation_parameters.seconds_per_timestep / 3.6e6 )
        else:
            price_injection_forecast_in_cent_per_kW_timestep = [0.1]
            price_purchase_forecast_in_cent_per_kW_timestep = [0.5]
        ...
        SingletonSimRepository().set_entry(
            ...
            entry=price_injection_forecast_in_cent_per_kW_timestep,
        )
        SingletonSimRepository().set_entry(
            key=SingletonDictKeyEnum.PRICEPURCHASEFORECAST24H,
            entry=price_purchase_forecast_in_cent_per_kW_timestep,
        )
        stsv.set_output_value(self.price_purchase_channel, price_purchase_forecast_in_cent_per_kW_timestep[0])
        stsv.set_output_value(self.price_injection_channel, price_injection_forecast_in_cent_per_kW_timestep[0])
    ...
    def i_prepare_simulation(self) -> None:
        ...
        tariff band matching `installed_capacity_in_kW`. Results are stored back on
        `self.price_signal_config` (fixed_price_in_cent_per_kW_timestep, static_tou_price_in_cent_per_kW_timestep, price_injection_in_cent_per_kW_timestep,
        ...
        if f"Fixed_Price_{country}" in price_purchase:
            ...
            fixed_price_in_cent_per_kW_timestep = price_purchase[f"Fixed_Price_{country}"].tolist()
            # convert euro/kWh to cent/kW-timestep
            p_conversion = 100 / (1000 * 3600 / self.my_simulation_parameters.seconds_per_timestep)
            fixed_price_in_cent_per_kW_timestep = [element * p_conversion for element in fixed_price_in_cent_per_kW_timestep]
            self.price_signal_config.fixed_price_in_cent_per_kW_timestep = np.repeat(
                fixed_price_in_cent_per_kW_timestep,
                int(3600 / self.my_simulation_parameters.seconds_per_timestep),
            ).tolist()

            static_tou_price_in_cent_per_kW_timestep = price_purchase[f"Static_TOU_Price_{country}"].tolist()
            static_tou_price_in_cent_per_kW_timestep = [element * p_conversion for element in static_tou_price_in_cent_per_kW_timestep]
            self.price_signal_config.static_tou_price_in_cent_per_kW_timestep = np.repeat(
                static_tou_price_in_cent_per_kW_timestep,
                int(3600 / self.my_simulation_parameters.seconds_per_timestep),
            ).tolist()
            ...
            for i in range(len(fit_data)):
                if (
                    fit_data["min_capacity (kW)"].values[i] < self.price_signal_config.installed_capacity_in_kW
                    and fit_data["max_capacity (kW)"].values[i] >= self.price_signal_config.installed_capacity_in_kW
                ):
                    price_injection_in_cent_per_kW_timestep = fit_data["FIT"].values[i]
            self.price_signal_config.price_injection_in_cent_per_kW_timestep = price_injection_in_cent_per_kW_timestep * p_conversion
        pass
