...
class PVSystem(cp.Component):
    ...
    def __init__(self, my_simulation_parameters: SimulationParameters, config: PVSystemConfig) -> None:
        ...
        self.electricity_outputC: cp.ComponentOutput = self.add_output(object_name=self.component_name, field_name=self.ElectricityOutput,
                                                                       load_type=lt.LoadTypes.ELECTRICITY, unit=lt.Units.WATT, postprocessing_flag=[lt.InandOutputType.ELECTRICITY_PRODUCTION], output_description=f"here a description for PV {self.ElectricityOutput} will follow.")
