...
class OccupancyConfig(cp.ConfigBase):
    ...
    @classmethod
    def get_scaled_chr01_according_to_number_of_apartments(cls, number_of_apartments: float) -> Any:
    ...
    def get_factors_from_country_and_profile(self) -> Tuple[float, float]:
        ...
        factor_electricity_consumption = float(scaling_factor_line["Unit consumption per dwelling for cooking (toe/dw)"]) * 1.163e4 + float(
            scaling_factor_line["Unit consumption per dwelling for lighting and electrical appliances (kWh/dw)"]
        )  # 1 toe = 1.163e4 kWh
        factor_hot_water_consumption = (
            float(scaling_factor_line["Unit consumption of hot water per dwelling (toe/dw)"]) * 4.1868e7 / ((40 - 10) * 0.977 * 4.182)
...
class Occupancy(cp.Component):
    ...
    def __init__(self, my_simulation_parameters: SimulationParameters, config: OccupancyConfig) -> None:
        ...
        if self.profile_name != "AVG" and self.my_simulation_parameters.year != 2021:
            raise Exception("LPG data is only available for 2021, if other years are needed, " + "use loadprofilegenerator_utsp_connector instead.")
        self.build()
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        stsv.set_output_value(
            ...
            self.heating_by_residents[timestep] * self.scaling_factor_according_to_number_of_apartments,
        )
        stsv.set_output_value(
            self.heating_by_devices_channel,
            self.heating_by_devices[timestep] * self.scaling_factor_according_to_number_of_apartments,
        )
        stsv.set_output_value(
            self.electricity_output_channel,
            self.electricity_consumption[timestep] * self.scaling_factor_according_to_number_of_apartments,
        )
        stsv.set_output_value(
            self.water_consumption_channel,
            self.water_consumption[timestep] * self.scaling_factor_according_to_number_of_apartments,
        )
    ...
    def build(self):
        ...
        if file_exists:
            ...
            water_consumption = pd.to_numeric(pre_water_consumption.loc[:, "Sum [L]"] * scaling_water_consumption).tolist()
            ...
            if self.profile_name == "AVG":
                ...
                heating_by_devices = pd.to_numeric(pre_heating_by_devices.loc[:, "Sum [kWh]"] * 1000 * 60).tolist()  # 1 kWh/min == 60W / min
            ...
            if minutes_per_timestep > 1:
                ...
                self.water_consumption = [
                    sum(self.water_consumption[n : n + minutes_per_timestep]) for n in range(0, steps_desired_in_minutes, minutes_per_timestep)
                ]
