...
class UtspLpgConnector(cp.Component):
    ...
    def get_profiles_from_utsp(
        ...
        if isinstance(lpg_households, JsonReference):
            ...
            raise TypeError(f"Type of lpg_households {type(lpg_households)} is invalid. It should be a JSONReference or a list of JSONReference.")
    ...
    def build(self):
        ...
        for list_index, list_item in enumerate(list_of_file_exists_and_cache_files):
            ...
            if not cache_complete or file_exists is False:
                log.information("LPG data cannot be taken from cache. It will be taken from UTSP or from predefined profile.")
    ...
    def get_result_lists_by_summing_over_value_dict(self, value_dict: Dict[Any, Any]) -> Tuple[List, List, List, List, List]:
    ...
    def calculate_one_lpg_request(self, simulation_config: HouseCreationAndCalculationJob, guid: str) -> Tuple[str, str, str, str, str, List[str]]:
        ...
        result = client.request_time_series_and_wait_for_delivery(self.utsp_url, request, self.utsp_api_key, timeout=100)
