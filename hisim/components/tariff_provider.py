...
class TariffProvider(cp.Component):
    ...
    def _purchase_price(self, timestep: int) -> float:
        """The marginal purchase price of this timestep, on the BEST_ESTIMATE slot (§8.4).
        ...
        return marginal_purchase_price_in_euro_per_kwh(self.contract, weekday, hour, spot).best_estimate

    def _injection_price(self) -> float:
        """The feed-in remuneration of one kWh, on the BEST_ESTIMATE slot (§8.4).
        ...
        Like every other output of this component it reports the BEST_ESTIMATE slot of the uncertainty
        ...
        """
        return self.contract.feed_in.rate_in_euro_per_kwh.best_estimate
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if capacity.kind != CapacityChargeKind.NONE and self.electricity_from_grid_input.source_output is not None:
            ...
            stsv.set_output_value(
                ...
                capacity.price_in_euro_per_kw.best_estimate if sets_new_peak else 0.0,
            )
        else:
