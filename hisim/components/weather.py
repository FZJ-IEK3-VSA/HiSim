    ...
    def i_prepare_simulation(self) -> None:
        ...
        if cachefound:
            ...
            tmy_data = read_test_reference_year_data(
                ...
                simulationparameters=self.my_simulation_parameters,
            )
...
def read_dwd_10min_data(filepath: str, simulation_parameters: SimulationParameters) -> pd.DataFrame:
    ...
    # get data
    data = pd.read_csv(filepath, encoding="utf-8", skiprows=[0, 1])
    data.index = pd.date_range(f"{simulation_parameters.year}-01-01 00:00:00", periods=24 * 6 * int(simulation_parameters.duration.days), freq="600S", tz="UTC")
    data = data.rename(
        columns={
