...
class ResolvedDynamicConnection:
    ...
    Every port this record names is named by a template, without exception: a dispatch block's
    signal comes out of a port resolution creates for it, never out of one the aggregator brought
    along. An aggregator used to arrive at the wiring stage already publishing a control output
    per participant class it declares a default feed for, and a dispatch block then took that port
    over rather than grow a second one no tag-and-weight lookup could tell from the first; those
    constructor-published ports are gone (F-1), and a claim on a signal an aggregator somehow
    still publishes is refused by the dispatch planner instead.
    ...
    def dispatch_output_name(self) -> Optional[str]:
        ...
        The port the signal comes out of and the port resolution creates are one and the same,
        so this is also what the aggregator and the port checks ask for when they ask which port
        they are responsible for bringing into existence. Two templates are in play: a dispatch
        block naming a target input produces ``DispatchTo{source}_{input}``; one without a target
        input — the recorded but unread signal — produces ``DispatchFor{source}_{output}``, which
        stays collision-free for the same reason the input names do.
