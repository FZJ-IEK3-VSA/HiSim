...
| `staged.py` | `Stage`, `StagedResult`, `StagedEvaluator`: one renovation plan spread over several years, priced as a **splice** of per-stage evaluations, with each earlier stage's equipment ageing into the next stage's register. Adds no money of its own -- a plan of one stage equals `EconomicEvaluator` entry for entry. |
| `staged_document.py` | `economics_result.json`: one staged plan in the shape a frontend draws from -- eight cost groups in a fixed order, per-subject build-up with the measure and the stage that bought it (and, per row, the investment each stage booked), annual series with markers, subsidies, financing and the comparison against doing nothing. Every document is validated against `economics_result.schema.json`, which ships beside it. |
