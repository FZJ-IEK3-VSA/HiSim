...
def _ordered_band(slots: dict) -> UncertainValue:
    """Builds a band from three independently computed slot values, restoring `min <= avg <= max`.
    ...
    the average is the meaning-preserving repair rather than a fudge.

    Args:
        slots: Mapping with the keys ``minimum``, ``average`` and ``maximum``.

    Returns:
        The band with `average` untouched and the outer slots ordered around it.
    """
    average = slots["average"]
    outer = (slots["minimum"], slots["maximum"])
    return UncertainValue(
        average=average, minimum=min(min(outer), average), maximum=max(max(outer), average)
    )
...
class DE2024Ruleset:
    ...
    def modernization_levy_entries(self, ctx: AllocationContext) -> List[CashFlowEntry]:
        ...
        LANDLORD entry for every year of its duration, so the pair nets to zero in the AVERAGE slot
...
def assert_zero_sum(
    ...
    **Envelope semantics** (decided 2026-08-12, cost-spec-v2 §7 B11; cost_spec.md §6.5 restates
    ...
    1. **AVERAGE slot: exact equality**, always — `sum(payer NPVs).average == system.average`;
    ...
    landlord's leg with ``L.as_revenue() = (-L.max, -L.avg, -L.min)`` — the mirror is forced by
    the band invariant ``minimum <= average <= maximum`` on signed amounts (§3.9: "minimum"
    ...
    ``(L.min - L.max, 0, L.max - L.min)`` — zero in AVERAGE, non-positive in `minimum`,
    ...
    if abs(system_npv.average - total.average) > _scaled_tolerance(system_npv.average):
        raise AssertionError(
            f"Zero-sum invariant violated in slot average: system={system_npv.average}, "
            f"payers={total.average}."
        )
