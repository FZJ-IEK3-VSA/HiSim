...
class EnergyPriceEntry:
    ...
    ``quantity_unit`` is the unit this row quotes prices in. All *shipped* files quote "kWh", so
    the `*_per_kwh` field names read literally true in the data; where a source published its
    price per liter or per ton, the row's ``notes`` record that as-published quote and the
    heating value it was divided by. A user-supplied file may still quote "liter" or "ton"
    natively: no engine code ever bills in those units — `CostDatabase.get_energy_price` divides
    the working price and the emission factor by the carrier's energy content and hands out a
    copy whose `quantity_unit` is "kWh" and whose `converted_from` records the division (D26).
