...
class CostDatabase:
    ...
    def get_energy_price(self, carrier: EnergyCarrier, year: int, country: str) -> EnergyPriceEntry:
        ...
        **The unit conversion happens here**, at resolution, and only here. Solid and liquid fuels
        are quoted per ton or per liter on disk because that is how the cited sources publish them,
        but every quantity in the engine is kWh, so the returned entry is a *copy* whose working
        ...
        row keeps its native quote, which is what keeps scenario overlay paths — and a reviewer
        comparing a row against its source — talking in the unit the literature uses.
    ...
    def conversion_detail(self, entry: EnergyPriceEntry, parameter_field: str) -> Optional[str]:
        ...
        Returns:
            ...
            described through their AVERAGE slot; the recorded value keeps the full band.
        """
        ...
        converted_value = converted.average if isinstance(converted, UncertainValue) else float(converted)
