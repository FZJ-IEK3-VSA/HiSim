...
a reconciliation that does not close, a band whose average sits outside its own bounds — and
...
def _npv_of(result: LifecycleCostResult, *categories: CostCategory) -> float:
    """AVERAGE-slot NPV of the given categories (the checks judge the expected world).
    ...
    AVERAGE slot is used: LOW and HIGH are the deliberately extreme corners of the envelope and
    would trip generous ranges for reasons that are not defects (§3.9).
    """
    return sum(
        result.npv_by_category[category].average
...
def _structural_findings(
    ...
    min <= avg <= max; the residual value never exceeds what was ever purchased; and support
    ...
    for carrier, quantities in reference.annual_energy_quantities_by_carrier.items():
        ...
        findings.append(
            _range_finding(
                ...
                bounds,
                "EUR/kWh",
                context={"year1_cost": year1.average, "quantity": quantity},
            )
        )
    return findings
