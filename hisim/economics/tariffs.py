...
class TariffContract:
    ...
    def from_json(cls, raw: dict, registry: Optional[SourceRegistry] = None) -> "TariffContract":
        ...
        `UncertainValue.from_json`, so a contract may state either an exact number or a min/avg/max
...
class Year1Bill:
    ...
    The three scalar fields are the §8.5 decomposition, all on the AVERAGE slot and all
...
def apply_tariff(determinants: BillingDeterminants, contract: TariffContract) -> Year1Bill:
    ...
    if something was sold) — plus the §8.5 decomposition scalars on the AVERAGE slot. Prices are
