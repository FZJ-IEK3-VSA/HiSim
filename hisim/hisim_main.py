...
def initialize_from_json(
    ...
    def import_from_string(full_classname: str):
        ...
        try:
            ...
            raise ValueError(f"Invalid class path: {full_classname}")
...
def run_simulation(my_sim: sim.Simulator, path_to_module: Optional[str]) -> None:
    ...
    for comp in my_sim.wrapped_components:
        log.information(f"Component {comp.my_component.component_name} has inputs {[input.full_name for input in comp.component_inputs]} and outputs {[output.full_name for output in comp.component_outputs]}")
...
def main_cli():
    try:
        ...
        if config["mode"] == "python":
            ...
            print(f"Running simulation of scenario {config['scenario']} with simulation parameters {config['simulation']}" + (f" and delta {config['delta']}" if config["delta"] else ""))
