...
def simulate(t_in_primary: any, t_in_secondary: any, parameters: pd.DataFrame,
    ...
    if group_id == 1 or group_id == 4:
        ...
        pass
    if (type(t_in) == pd.core.series.Series or type(t_in_secondary) == pd.core.series.Series or type(
        ...
        if group_id == 1 or group_id == 2 or group_id == 3:
            ...
            df.loc[df['Modus'] == 1, 'P_el'] = (p1_p_el * t_in + p2_p_el * df[
                'T_out'] + p3_p_el + p4_p_el * t_amb) * p_el_ref  # this is the first calculated value for P_el
            if group_id == 1:  # with regulated heatpumps the electrical power can get too low. We defined a minimum value at 25% from the point at -7/output temperature.
                ...
                df.loc[df['Modus'] == 2, 'P_el'] = (p5_p_el * t_in + p6_p_el * df[
                    'T_out'] + p7_p_el + p8_p_el * t_amb) * p_el_col_ref
                df.loc[(df['Modus'] == 2) & (df['T_in'] < 25), 'P_el'] = p_el_col_ref * (
            ...
            if group_id == 2:
                df['t_in'] = df['T_in']
                df.loc[:, 't_amb'] = -7
            df.loc[(df['Modus'] == 1) & (df['P_el'] < 0.25 * p_el_ref * (
        ...
        if self.group_id in (1, 2, 3):
            ...
            if modus == 2:
                ...
                if t_in < 25:
                    ...
                    t_amb = t_in
                p_el = (
                                   self.p5_p_el * t_in + self.p6_p_el * t_out + self.p7_p_el + self.p8_p_el * t_amb) * self.p_el_col_ref
                if p_el < 0:
                    eer = 0
                    p_el = 0
                p_th = -(eer * p_el)
