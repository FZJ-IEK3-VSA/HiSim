...
def test_occupancy():
    ...
    for i in range(24 * 60 * 365):
        ...
        number_of_residents.append(stsv.values[my_occupancy.number_of_residents_channel.global_index])
        heating_by_residents.append(stsv.values[my_occupancy.heating_by_residents_channel.global_index])
        heating_by_devices.append(stsv.values[my_occupancy.heating_by_devices_channel.global_index])
        electricity_consumption.append(stsv.values[my_occupancy.electricity_output_channel.global_index])
        water_consumption.append(stsv.values[my_occupancy.water_consumption_channel.global_index])
