        ...
        if output.postprocessing_flag is not None:
            if InandOutputType.WATER_HEATING in output.postprocessing_flag:
                if LoadTypes.DISTRICTHEATING in output.postprocessing_flag:
                    csv_frame[('WaterHeating', 'Distributed Stream [kWh]')] = sum(results.iloc[:, index]) * 1e-3
                elif LoadTypes.GAS in output.postprocessing_flag:
                    csv_frame[('WaterHeating', 'Gas [kWh]')] = sum(results.iloc[:, index]) * 1e-3
                elif LoadTypes.OIL in output.postprocessing_flag:
                    csv_frame[('WaterHeating', 'Oil [l]')] = sum(results.iloc[:, index])
                else:
                    csv_frame[('WaterHeating', 'Electricity [kWh]')] = compute_energy_from_power(power_timeseries=results.iloc[:, index],
                ...
                if LoadTypes.DISTRICTHEATING in output.postprocessing_flag:
                    csv_frame[('SpaceHeating', 'Distributed Stream [kWh]')] = sum(results.iloc[:, index]) * 1e-3
                elif LoadTypes.GAS in output.postprocessing_flag:
                    csv_frame[('SpaceHeating', 'Gas [kWh]')] = sum(results.iloc[:, index]) * 1e-3
                elif LoadTypes.OIL in output.postprocessing_flag:
                    csv_frame[('SpaceHeating', 'Oil [l]')] = sum(results.iloc[:, index])
                else:
                    csv_frame[('SpaceHeating', 'Electricity [kWh]')] = compute_energy_from_power(power_timeseries=results.iloc[:, index],
                    seconds_per_timestep=simulation_parameters.seconds_per_timestep)
