...
def capex_calculation(
    ...
    lines = []
    lines.append("component_name, investment_costs_per_simulated_period, device_co2_footprint_per_simulated_period, lifetime")

    for component in components:
        component_unwrapped = component.my_component
        capex, co2_footprint, lifetime = component_unwrapped.get_cost_capex(component_unwrapped.config)

        if lifetime > 0:
            # lifetime is per default set to 1.0 in class cp.Component to avoid devide by zero error
            capex_per_simulated_period = (capex / lifetime) * (simulation_parameters.duration.total_seconds() / seconds_per_year)
            device_co2_footprint_per_simulated_period = (co2_footprint / lifetime) * (simulation_parameters.duration.total_seconds() / seconds_per_year)
    ...
    lines.append(f"investment cost per simulated periond: {total_investment_cost_per_simulated_period:3.0f} EUR")
    lines.append(f"CO2-emissions for production of all devices per simulated periond: {total_device_co2_footprint_per_simulated_period:3.0f} kg")
