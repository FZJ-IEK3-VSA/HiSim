...
class ResultDataCollection:
    ...
    def __init__(
        ...
        folder_from_which_data_will_be_collected: str = os.path.join(os.pardir, os.pardir, os.pardir, "system_setups", "results"),
    ...
    def filter_results_that_failed_to_heat_or_cool_building_sufficiently(
            ...
            for folder in list_of_result_path_that_contain_scenario_data:
                    ...
                    with open(scenario_data_information, "r", encoding="utf-8") as data_info_file:
                        ...
                        for component in component_entries:
                            if "Building" in component["componentName"]:
                                set_heating_temperature = float(component["configuration"].get("set_heating_temperature_in_celsius"))
                                set_cooling_temperature = float(component["configuration"].get("set_cooling_temperature_in_celsius"))
                                break
                else:
                ...
                if os.path.exists(webtool_kpis_file):
                    with open(webtool_kpis_file, "r", encoding="utf-8") as kpi_file:
                        ...
                        min_temperature = float(kpi_data.get("Minimum building indoor air temperature reached [\u00b0C] "))
                        max_temperature = float(kpi_data.get("Maximum building indoor air temperature reached [\u00b0C] "))
                        ...
                        if min_temperature <= set_heating_temperature - 5.0 and max_temperature >= set_cooling_temperature + 5.0:
                            file.write(
                        ...
                        elif min_temperature <= set_heating_temperature - 5.0 and max_temperature < set_cooling_temperature + 5.0:
                            file.write(
                                "too cold,"
                                f"{set_heating_temperature},"
                                f"{set_cooling_temperature},"
        ...
        print(f"Total number of simulations that have building temperatures way below or above set temperatures: {len(list_of_unsuccessful_folders)}")
        ...
        if answer.upper() in ["N", "NO"]:
            ...
            print("The folders with too low or too high building temperatures will be discarded from the further analysis.")
    ...
    def get_list_of_all_relevant_scenario_data_folders(self, result_path: str) -> List[str]:
        ...
        list_with_all_paths_to_check = list_of_paths_first_order + list_of_paths_second_order + list_of_paths_third_order
    ...
    def make_dictionaries_with_simulation_duration_keys(
        ...
        if bool(dict_of_csv_data) is False:
            raise ValueError("The dictionary is empty. Maybe no data was collected. Please check your parameters again.")
    ...
    def read_csv_and_generate_pandas_dataframe(
        ...
        for csv_file in csv_data_list:
            ...
            if rename_scenario is True:
                if parameter_key is not None and list_with_parameter_key_values is not None and list_with_parameter_key_values != []:
                    ...
                    dataframe["scenario"] = self.rename_scenario_name_of_dataframe_with_index(dataframe=dataframe, index=index)
    ...
    def go_through_all_scenario_data_folders_and_check_if_module_configs_are_double_somewhere(
            ...
            for file in os.listdir(folder):
                if ".json" in file:
                    with open(os.path.join(folder, file), "r", encoding="utf-8") as openfile:  # type: ignore
                        config_dict = json.load(openfile)
                        my_module_config_dict = config_dict["myModuleConfig"]
                        my_module_config_dict.update({"duration in days": config_dict["scenarioDataInformation"].get("duration in days")})
        ...
        print(f"The following folders seem to be duplicated: {list_with_duplicates}. Number: {len(list_with_duplicates)}.")
