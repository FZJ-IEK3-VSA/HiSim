...
class ScenarioDataProcessing:
    ...
    @staticmethod
    def filter_pandas_dataframe_according_to_output_variable(dataframe: pd.DataFrame, variable_to_check: str) -> pd.DataFrame:
    ...
    def check_if_scenario_exists_and_filter_dataframe_for_scenarios_dict(
        ...
        for scenario_to_check_key, list_of_scenarios_to_check in dict_of_scenarios_to_check.items():
            log.information(f"Scenarios to check are {list_of_scenarios_to_check} in Input column {scenario_to_check_key}.")
        ...
        if filter_level_index > 1:
            raise ValueError(f"Filter level index is {filter_level_index} but should be 1 at max."
    ...
    def take_mean_values_of_scenarios(
        ...
        if len(x_data) == 0:
            raise ValueError(f"x data length is 0. This means no keys for output values could be found in the columns of filtered data: {filtered_data.columns}")
...
class OutputVariableEnumClass(Enum):
    ...
    KPI_DATA = DataInfo(descriptions=[
    ...
    ELECTRICITY_DATA = DataInfo(descriptions=[
    ...
    FLOW_AND_RETURN_TEMPERTAURES = DataInfo(descriptions=[
...
class ScenarioAggregationEnumClass(Enum):
    ...
    BUILDING_TYPE = DataInfo(descriptions=[
        "SFH",
        "TH",
        "MFH",
        "AB"
    ])
    BUILDING_TYPE_AND_AGE = DataInfo(descriptions=[
        # "DE.N.TH.10",
        # "DE.N.TH.09",
        # "DE.N.TH.08",
        # "DE.N.TH.07",
    ...
    BUILDING_REFURBISHMENT_STATE = DataInfo(descriptions=[
        "001.001",
        "001.002",
        "001.003"
    ])

    BUILDING_AGE = DataInfo(descriptions=[
        "01.Gen",
        "02.Gen",
        "03.Gen",
        "04.Gen",
