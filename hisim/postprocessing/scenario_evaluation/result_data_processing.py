...
class ScenarioDataProcessing:
    ...
    def filter_pandas_dataframe(dataframe: pd.DataFrame, variable_to_check: str) -> pd.DataFrame:
        ...
        if filtered_dataframe.empty:
            ...
            # raise ValueError(
            print(f"The filtered dataframe is empty. The dataframe did not contain the variable {variable_to_check}. Check the list above.")
    ...
    def aggregate_all_values_for_one_scenario(
        ...
        for scenario_to_check in list_of_scenarios_to_check:
            ...
            for value in dataframe[column_name_to_check].values:
                if isinstance(scenario_to_check, str) and scenario_to_check in value and value not in aggregated_scenario_dict[scenario_to_check]:
        ...
        for (
            ...
            df_filtered_for_specific_scenarios = dataframe.loc[dataframe[column_name_to_check].isin(given_list_of_values)]
    ...
    def calculate_relative_electricity_demand(dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        for scenario in list(set(filtered_data.scenario.values)):
            if "share_of_maximum_pv_power" not in scenario:
                ...
                df_for_one_scenario_and_for_share_zero = df_for_one_scenario.loc[df_for_one_scenario.share_of_maximum_pv_power == 0]
                ...
                if share_of_maximum_pv_power == 0:
                    ...
                    df_for_one_scenario_and_for_share_zero = filtered_data.loc[filtered_data.share_of_maximum_pv_power == 0]
                    reference_value_for_electricity_demand = df_for_one_scenario_and_for_share_zero.value.values

                    # calculate reference electricity demand for each scenario and share of pv power
                    relative_electricity_demand = (
                        1 - ((reference_value_for_electricity_demand - value_for_electricity_demand) / reference_value_for_electricity_demand)
                    ) * 100
