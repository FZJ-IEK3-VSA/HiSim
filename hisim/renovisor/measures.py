...
def _apply_single_measure(result: MeasureApplication, path: str, measure_type: str, params: dict[str, Any]) -> None:
    ...
    if measure_type == "heat_pump":
        ...
        power_in_kilowatt_peak = _numeric_param(params, "kWp")
        if power_in_kilowatt_peak is None:
            result.report_notes.append((path, "ignored", "pv measure without a numeric 'kWp' param"))
            return
        home.setdefault("pv", {})["kWp"] = power_in_kilowatt_peak
        result.report_notes.append((path, "used", f"pv.kWp set to {power_in_kilowatt_peak} (new total)"))
    elif measure_type == "battery":
        energy_in_kilowatt_hour = _numeric_param(params, "kWh")
        if energy_in_kilowatt_hour is None:
            result.report_notes.append((path, "ignored", "battery measure without a numeric 'kWh' param"))
            return
        home.setdefault("battery", {})["kWh"] = energy_in_kilowatt_hour
        result.report_notes.append(
            (path, "approximated", f"battery.kWh set to {energy_in_kilowatt_hour} (new total); size is auto-sized by the setup")
        )
