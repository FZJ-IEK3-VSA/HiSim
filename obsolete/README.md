...
| `components/chp_config_advanced.py` | D-16 | `CHPConfigAdvanced`, moved out of `hisim/components/advanced_fuel_cell.py`, which keeps `CHPConfig` and `CHP`. Zero call sites, and not a configuration in this repository's sense: a plain class rather than a `ConfigBase` dataclass, so it could carry no preset and reach no energy-system file. It read one row of `hisim/inputs/chp_system/mock_up_efficiencies.xlsx` ("BlueGen BG15", five other machines commented out beside it) at construction, which is why its whole body sat inside `__init__`. The spreadsheet stays in `hisim/inputs`: nothing reads it now, but it is the only record of those six machines. |
