...
- **W2.6 — `legacy_flat_subsidy_share`. Done** (commit 8696c966), by marking
  ...
  **Retired 2026-09-24** (owner decision on the PR #799 review): the flat shim
  is gone. With no catalogue nothing is booked, on every path (evaluator,
  postprocessing bridge, `hisim.economics evaluate`, reports, scenarios), and
  no `LEGACY_MIGRATION_SHIM` record is written any more. The
  `legacy_flat_subsidy_share` field is kept in the device data and in
  `DeviceEntry` until its removal, but nothing reads it for pricing.
...
| D19 | Shim / parity formula / explain | Ratified as implemented: legacy flat-subsidy shim marked-not-moved and removed from the overlay surface (the shim itself retired 2026-09-24, see W2.6); parity audit keeps the bare-device-investment formula; `explain` lists scoped entries for scoped figures (B4). |
