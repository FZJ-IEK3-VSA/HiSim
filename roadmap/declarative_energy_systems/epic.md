...
*Decision.* `[decided 2026-08-26, owner]` **(a)** — many-cardinality stays a declared hook (`Many(...)` raises `NotImplementedError`) until the first real consumer is converted (plan parking lot); the file syntax for it (a list under `sizing_sources`) is fixed now. UC2's EMS lists stay a format test until then.
