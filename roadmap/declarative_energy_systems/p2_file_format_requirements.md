...
*Amended.* `[decided 2026-09-12, owner]` The v1 files retired **in P4, not P5**: the twenty-two `system_setups/*.scenario.json` twins, `json_executor.py`, `hisim_convert_to_json.py`, `scripts/regenerate_scenario_jsons.py`, `scenario-json-freshness.yml` and `golden-json-check.yml` were deleted in one pull request from main. What made it early rather than late: every setup has a recorded v3 twin, and `energy-system-freshness`, `golden-yaml-check` and the parity rig give the surviving representation the guarantees the JSON pair gave the retiring one — while the JSON pair cost about 48 CI minutes per pull request plus a regeneration step on every setup-changing PR for the whole of the P4 sweep. `system_setups/` keeps the Python setups and the shared `*.simulation.json` parameter files, which both surviving modes read; its directory name is still a later decision.
