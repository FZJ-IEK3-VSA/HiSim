...
| AC-P3.20 `[deferred to P6, 2026-08-31]` | The phase-6 teardown deletes the workflow, its configuration and its scripts, and the repository contains no reference to them afterwards. It is an acceptance criterion of P6, not of P3 — P3 ships the rig, P6 removes it. | R11.8 | → hisim-b3b.22
...
| Q-P3.4 | Where do recorded files live, and do the v1 JSON twins stay? | R9, R4, AC-P3.10 | `[answered 2026-08-28; retired 2026-09-12]` (a) `energy_systems/<stem>.energy_system.yaml`, v1 twins and their workflow untouched; v1 retires with `json_executor.py` in P5 — **done 2026-09-12, brought forward into P4** (owner): the twins, `json_executor.py`, `hisim_convert_to_json.py` and both JSON workflows are deleted, since every setup has a recorded twin and the YAML gates cover what the JSON pair covered |
