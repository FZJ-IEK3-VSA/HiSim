...
**Answered 2026-09-10: (a)** — the 42/50 and 35/31 asymmetry is a bug; normalise, ship `gas` and `hydrogen` with one shared buffer override, and let `test_generic_chp` change and say why.
**Reversed 2026-09-11 to (b)** on the first review of #683: the normalisation rested on inference alone, so the four factories keep their 2023 values, the asymmetry is recorded as unexplained, and the class converts as four presets (`gas`, `hydrogen`, `gas_with_buffer`, `hydrogen_with_buffer`) — the survey's own reading of (b). No physics change; `tests/test_generic_chp.py` pins the literals instead.
...
**Answered 2026-09-10: (a)** — adopt the `4 m²/apartment` law and record the diff; `household_gas_solar_thermal`'s week golden is re-blessed in the same commit.
...
10. **Hazards.** **27 of the 49 fields are lists — 13 runtime result buffers and 12 forecast inputs, none of which belongs in a file, plus two COP/EER coefficient vectors**. The 25 are runtime state, not configuration — `optimal_cost_in_eur`, `revenues_in_eur`, `pv2load_in_watt`, `battery_control_state`, `batt_soc_normalized_timestep`, all `*_forecast_24h_1min_*` … each defaulted to `[]` by the factory. Serialising them into an energy‑system file would freeze empty lists as wire format and write full‑year arrays into a realized record. Any conversion must first move them off the config. No mutable *dataclass* defaults (all 49 fields mandatory), so no shared‑list bug — but only by accident.
...
**Answered 2026-09-10: (b)** — fix the physics, own commit with result diffs; the five golden sizers are re-blessed at +10 % and `basic_household_only_heating` (+54 %) joins the week gate in the same commit, so the change is not silent.
...
`SimpleHotWaterStorageController(+Config)` (0 call sites at all), `MpcController` (0 setups; 27 of its 49 config fields are lists — 13 runtime result buffers and 12 forecast inputs, plus two coefficient vectors), `PIDController` (0 setups), `Windturbine` (0 setups), `PriceSignal` (0 setups), `RandomNumbersConfig.get_default_config` (0), `PVSystem.get_default_config` (0), and the five dead `configuration.py` classes. Options: (a) **Delete** the storage controller, the `configuration.py` five and the three zero‑call‑site factories now, and defer MPC/PID/wind/price to the runtime‑SimRepository redesign in the parking lot; (b) **convert everything**, minting ~9 preset names for code no setup runs; (c) delete all of them including the four components.
*Consequence:* (a) removes the storage controller's copy of the dead singleton read for free and keeps the wire vocabulary honest, while leaving four components in limbo; (b) freezes nine names at P5 and forces `MpcControllerConfig`'s 13 result buffers and 12 forecast inputs into the file format; (c) removes the only MPC/PID prototypes in the repo.
**Answered 2026-09-10, beyond the three options offered: everything to `obsolete/`, nothing deleted.** The four components move now with their tests rather than being deferred, and each class that shared a file (`SimpleHotWaterStorageController`, the four dead `configuration.py` classes) is moved out into its own file under `obsolete/components/`. `HouseholdWarmWaterDemandConfig` was found to be live and stays; `HydrogenStorageConfig` and `AdvElectrolyzerConfig` stay for D-25/D-29. Only the two dead factories are deleted — a factory is code, not a class.
...
2. **status: converted 2026‑08‑31 (D‑23 answered).** The component *is* buildable by the executor now: `Car.__init__` takes `(my_simulation_parameters, config)` and nothing else, and the driving profile reaches it through the per‑simulation `SimRepository` instead of through its constructor. What survives of the objection below is entirely correct and unchanged — a two‑series‑per‑car payload can never be a `SizingContext` fact — and the design routes *around* that rather than through it: the config carries the payload's **identity** (`household_name`, `car_name`), the occupancy publishes the payload under that identity in its `i_prepare_simulation`, and the car looks it up in its own. `GenericCarInformation` moved to the new leaf module `hisim/components/lpg_car_information.py` so that the occupancy can use it to publish without importing the car.
3. **Factories.**
...
4. **Presets → one constructor, `for_household` (deviation, 2026‑08‑31).** The supplement asked for `diesel` and `electric`. Neither can be a *preset*: a preset takes nothing but the instance name, and a car cannot be built without naming the household and car whose profile it drives by. The two fuels became a `fuel` parameter of `for_household(name, household_name, car_name, fuel=…, source_weight=…)` instead, which mints one wire name rather than two and keeps the nine values of `get_default_ev_config` and `get_default_diesel_config` bit‑for‑bit (maintenance is now derived as 2 % of the investment cost, which is what both factories spelled out).
...
7. **Behaviour: neutral** — `for_household(fuel=ELECTRICITY)` reproduces `get_default_ev_config`'s nine values exactly, and `fuel=DIESEL` those of `get_default_diesel_config`; verified by the parity rig, which compares this setup's every result column and every KPI at exact equality (both windows, 2026‑08‑31). The setup is smoke‑gated only (`tests/test_system_setups_households_for_building_sizer.py:277-279`, `@pytest.mark.system_setups`) and is **not in the 8‑setup golden fleet** — any change here is numerically unwitnessed.
...
10. **Hazards.**
    - ~~**`Car.__init__` takes a third mandatory argument**, `data_dict_with_car_information: Dict` (`:242`)~~ — **removed 2026‑08‑31.** The observation was exactly right: the executor calls `component_class(my_simulation_parameters=…, config=…)` and nothing else (`wiring.py:143-145`), and `Car` was the only component in group C a v3 file could not construct. The argument is gone; the profile arrives through the repository in `i_prepare_simulation`. What the conversion of `CarConfig` bought on its own was indeed nothing — the constructor‑argument removal is what made the car expressible, and the config conversion is what gave the removal something to look the profile up by.
    - ~~**Config aliasing in the loop.**~~ **Fixed 2026‑08‑31**: the loop now calls `for_household` per car, so each car owns its configuration. Previously `my_car_config` was built **once** at `:396` and mutated **inside** the loop (`:403`), then handed to every `Car`; all cars share one `CarConfig` instance whose `component_id` ends as the last car's. Component *names* differ because `get_component_name()` (`hisim/component.py:269`) is evaluated during `super().__init__`, but every `car.config` afterwards reports the last car's identity — which is exactly what a recorder would write. Latent today only because CHR01 + `Bus_and_one_30_km_h_Car` yields one car.
...
**Answered 2026-09-10: (c)** — plumb the fact as a `WeatherConfig` contribution from a per-station DIN 12831 table and keep the Building's field a plain `-7.0` default: no result change. It is the repository's first two-provider fact, so a two-station district needs a `sizing_sources` line.
...
**Answered 2026‑08‑31 — (d), none of the three.** The owner chose a fourth option the survey did not list: *hand the profile over through the repository and put its identity in the config.* The occupancy publishes every profile it produced into the per‑simulation `SimRepository` during `i_prepare_simulation`; `Car` reads its own there in the same phase and loses its third constructor argument; `CarConfig` gains `household_name` and `car_name` plus a `for_household` constructor. This is (b)'s outcome — a recordable car setup — without (b)'s cost: no per‑car CSV has to be written by anybody, because the profile never leaves the process it was computed in. The survey's objection was **right and is not overturned**: the payload is two full time series and could not enter `SizingContext`; the design routes around `SizingContext` rather than through it, which is why no fact and no sizing law appear anywhere in it. Two consequences the survey named come with it. The count of cars is still a live LPG result, so the setup keeps its Python loop and the R5 template layer is still what a file would need to express N cars; and ordering becomes load‑bearing, since `i_prepare_simulation` runs in registration order and a car prepared before its occupancy would find nothing — which the lookup answers with a refusal naming the household, the car and every profile that *was* published, never with a silently stationary vehicle. `GenericCarInformation` moved out of `generic_car.py` into the leaf module `hisim/components/lpg_car_information.py`, because the publisher is now the occupancy and leaving the class where it was would have made `loadprofilegenerator_utsp_connector` import `generic_car`, which imports it back.

**Parity evidence, and the one thing it does not reach (2026‑08‑31).** Both triples agree on the wiring and on every KPI, and on 149 of the 150 shared result columns at exact equality. The one column that differs is `L2EMSElectricityController - TotalElectricityConsumption`, in 2 of 10 080 rows in each window, by at most 1.8e‑12 W — a relative 4.16e‑16, which is one to two units in the last place. The cause is not the car: it is that `L2GenericEnergyManagementSystem` sums its EMS‑controlled consumers in `my_component_inputs` *insertion* order, and the two wiring paths insert in different orders. Measured on this setup, the imperative path sums `L1EVChargeControl_1(w5), UTSPConnector(w1), …SH(w2), …DHW(w3), Battery(w6)` — the car's feed comes first because the setup adds it by hand before `prepare_calculation` applies the default connections — while the declarative resolver produces strict weight order, `w1, w2, w3, w5, w6`. Floating‑point addition is not associative, so two of ten thousand sums land one bit apart. `household_heatpump_building_sizer`, which adds no EMS feed by hand, passes the same rig at exact equality on the same commit, which is what pins the cause to the ordering rather than to the car conversion. **The car setup is simply the first to expose it.** The fix is to sum in a canonical order — by source weight, which both paths already agree on and which `inputs_sorted` already uses — but that changes the numbers of every setup whose EMS insertion order is not already weight‑ascending, so under R5 it is a named behaviour change of its own with a fleet‑wide diff, not something to slip into this one. **Left behind for a follow-up:** `UtspLpgConnectorConfig.cars` is now written by nobody and read by nobody — it existed only so the v1 executor could tell which occupancy a car belonged to. Removing the field would rewrite the `cars: null` line of all 21 recorded twins and every scenario JSON, which is a mechanical change worth making on its own rather than inside this one.
...
**Answered 2026-09-11: (a)** — moved to `obsolete/components/generic_smart_device.py` with `SmartDeviceConfig`, not deleted; executed the same day, and §6's copy-law candidate is retired with it.
