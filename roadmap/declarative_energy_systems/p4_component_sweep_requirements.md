...
| `SimpleHotWaterStorageConfig` | conv | `buffer` (+ `sizing_option: HotWaterStorageSizingEnum` field, D-10) | `volume_heating_water_storage_in_liter` ← **generator** `maximal_thermal_power_in_watt` × k (20/40/50 l/kW) — C11 executed 2026-09-11, the twelve setups pass the generator's power | — | **P** — C11 done (3 twins +10 %, `basic_household_only_heating` +54 %, 9 twins byte-identical); the conversion itself is behaviour-neutral | D-9, D-10, D-17 |
