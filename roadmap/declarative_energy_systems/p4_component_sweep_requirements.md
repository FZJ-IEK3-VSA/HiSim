...
2. **`MAIN_CLASS` on `ConfigBase`.** Every config class carries an eight-line
   ...
   *Enforced from 2026-09-17:* `ConfigBase.__init_subclass__` refuses a subclass that declares
   neither a non-empty `MAIN_CLASS` nor its own `get_main_classname`, so a configuration without an
   identity fails at the import that defines it instead of at the first thing that serializes it.
   The override survives as the exception a class states deliberately — the test doubles that
   configure no real component — and no configuration under `hisim/components/` uses it any more.
