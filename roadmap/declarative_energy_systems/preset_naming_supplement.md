...
| `BuildingConfig` (`building/config.py`) | `german_single_family_home` + constructor (was `standard`; conflict 8 reversed 2026-09-14) | `get_default_german_single_family_home` | `absolute_conditioned_floor_area_in_m2`, `number_of_apartments`, `max_thermal_building_demand_in_watt` and the 5 `*_u_value_*`/`*_area_in_m2` pairs | The `config_presets` branch ships `german_single_family_home`, which contradicts the Q-P1.6 refinement (§conflict 8) |
...
8. **A shipped preset name violates the Q-P1.6 refinement** (`BuildingConfig.presets.german_single_family_home` on `config_presets`). Rule 5 + Q-P1.6. Building is a TABULA-code lookup and must expose exactly one preset named `standard`. *Resolution:* rename to `standard` before the sweep, while nothing outside the branch depends on it; `german_single_family_home` becomes the docstring of the default `building_code`. **Reversed 2026-09-14** (R4 amendment): the archetype's name is what a reader of a twin wants to see, and rule 5's `standard` is reserved for presets nothing describes better. The preset is `german_single_family_home` again; `for_tabula_code` stays the general case. The same amendment renamed `aachen`, `couple_both_at_work`, `sized_to_pv` and `building_derived`.
