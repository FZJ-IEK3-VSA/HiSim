...
| F3 | `HISIM_IN_DOCKER_CONTAINER` makes `postprocessing_main.py` whitelist post-processing options, and `COMPUTE_LIFECYCLE_COSTS` is not on the list: a container with that variable set produces a `result.json` whose whole `costs` block is `missing`. | **HiSim change** before the container ships: add `COMPUTE_LIFECYCLE_COSTS` to the whitelist (deployment decision, not RenoVisor-owned code). |
