...
`setup_function` and wires no components. The component graph is entirely the existing
`household_heatpump_building_sizer.py` setup, which `initialize_from_python` imports and whose
`setup_function` it calls; this file is the *driver* that attaches the economics — parameters,
context, post-processing option — and starts the run. That separation is the point: the physics
...
def build_economic_context() -> EconomicContext:
    ...
    Assembles everything the simulation itself cannot know and hands it to the engine in one
    object (`bridge.EconomicContext`): what already stands in the building, who is applying for
    which subsidy, which non-simulated envelope measures belong to the same package, how the
    building is used, and which scenario axes to sweep. Each block below switches on a different
    part of the engine, which is what makes this function the readable core of the example.

    The existing-asset register is the most consequential piece: its mere presence flips the
    evaluated perspective bundle from greenfield to the full brownfield set (owner/landlord/
    tenant/macroeconomic) and enables residual values, removal costs and the anyway-cost credit.
    The subsidy context supplies the answers the BEG eligibility conditions resolve against, and
    the envelope measures enter as `SubjectCostFacts` because no simulation component represents
    them — they are sized in m² and reach the building physics only as changed U-values.

    Returns:
        The context to attach via `SimulationParameters.set_economic_context`.
    """
    ...
    old_top_ceiling = ExistingAsset(
        asset_class=ComponentType.TOP_CEILING_UPPER_INSULATION,
        size=90.0,
        size_unit=Units.SQUARE_METER,
        installation_year=1988,
        replaced_by_asset_classes=[ComponentType.TOP_CEILING_UPPER_INSULATION],
    )
    ...
    subsidy_context = SubsidyContext(
        ...
        building=SubsidyBuildingContext(
            ...
            has_renovation_roadmap=True,  # +5 % on envelope measures
    ...
    scenario_set = ScenarioSet.from_json(
        {
            ...
            "axes": [
                ...
                },
                {
                    ...
                    "levels": {"cheap": {"min": 800, "best_estimate": 1050, "max": 1450}},
                },
...
def main() -> None:
    ...
    Shows the three lines it takes to put a lifecycle-cost evaluation on top of an existing HiSim
    ...
    artifacts alongside the usual results. The final prints name the result directory, because
    that path is the entry point for every offline follow-up (`evaluate`, `explain`, `report`).
    """
    ...
    params.set_economic_parameters(
        EconomicParameters(
            ...
            subsidy_catalog_path=SubsidyCatalog.DEFAULT_PATH,  # activates the BEG engine
        )
    )
