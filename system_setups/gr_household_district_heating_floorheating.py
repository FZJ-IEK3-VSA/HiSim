"""Golden reference system setup: district heating with floor heating distribution.
...
def setup_function(
    ...
    This setup is specifically intended for district heating scenarios. The
    battery and the Energy Management System are only added when a non-zero PV
    share is configured and ``use_battery_and_ems`` is enabled; otherwise the
    electricity meter is connected directly to the simulation.
    ...
    Raises:
        ValueError: If the configured heat distribution system type is neither
            floor heating nor a radiator, or if the ``lpg_households`` config
            list is empty.
    ...
    energy_system_config_.heating_system = HeatingSystems.DISTRICT_HEATING
