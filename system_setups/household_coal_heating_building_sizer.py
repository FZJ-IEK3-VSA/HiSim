"""Household system setup with coal heater."""
...
__authors__ = "Sumit Gargesh"
__copyright__ = "Copyright 2024, FZJ-IEK-3"
__credits__ = ["Noah Pflugradt"]
__license__ = "MIT"
__version__ = "1.0"
__maintainer__ = "Sumit Gargesh"
...
def setup_function(
    ...
    This setup function emulates a household including the following components:
    ...
    # try reading energy system and archetype configs
    my_config = read_in_configs(my_sim.my_module_config)
    if my_config is None:
        my_config = ModularHouseholdConfig().get_default_config_for_household_coal()
        my_sim.my_module_config = my_config.to_dict()
        log.warning(
            f"Could not read the modular household config from path '{config_filename}'. Using the coal household default config instead."
        )
    ...
    if heating_system != HeatingSystems.COAL_HEATING:
        raise ValueError("Heating system needs to be coal heater for this system setup.")
    ...
    # Build Coal Heater
    # Coal boiler is conventional (no condensing), on-off operation with minimum run/idle times
    ...
    my_sim.add_component(my_coal_heater, connect_automatically=True)

    # Build Coal Heater Controller
    # Coal boiler cannot modulate and has significant minimum run/idle times
    my_coal_heater_controller_config = generic_boiler.GenericBoilerControllerConfig.get_default_coal_controller_config(
        minimal_thermal_power_in_watt=my_coal_heater_config.minimal_thermal_power_in_watt,
        maximal_thermal_power_in_watt=my_coal_heater_config.maximal_thermal_power_in_watt,
    ...
    my_coal_heater_controller = generic_boiler.GenericBoilerController(
        my_simulation_parameters=my_simulation_parameters,
        config=my_coal_heater_controller_config,
    )
    my_sim.add_component(my_coal_heater_controller, connect_automatically=True)
    ...
    # Buffer storage is important for coal heating, as it operates on-off and cannot modulate
    ...
    # Build Fuel Meter (tracks coal consumption)
    my_fuel_meter_config = fuel_meter.FuelMeterConfig.get_fuel_meter_default_config(
        fuel_loadtype=lt.LoadTypes.COAL,
        heating_value_of_fuel_in_kwh_per_liter=my_coal_heater.heating_value_of_fuel_in_kwh_per_liter,
        fuel_density_in_kg_per_m3=my_coal_heater.fuel_density_in_kg_per_m3,
    )
    ...
    # when no PV is used, connect electricity meter automatically
    else:
