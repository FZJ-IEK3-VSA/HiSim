...
def setup_function(
    ...
    my_hydrogen_boiler_config = generic_boiler.GenericBoilerConfig.preset_hydrogen("CondensingHydrogenBoiler").resolve(
        SizingContext(
            heating_load_in_watt=my_building_information.max_thermal_building_demand_in_watt,
    ...
    my_hydrogen_boiler_controller_config = generic_boiler.GenericBoilerControllerConfig.preset_modulating(
        "ModulatingBoilerController"
    ).resolve(
        SizingContext(
            minimal_thermal_power_in_watt=concrete(my_hydrogen_boiler_config.minimal_thermal_power_in_watt),
            maximal_thermal_power_in_watt=concrete(my_hydrogen_boiler_config.maximal_thermal_power_in_watt),
        )
    )
    my_hydrogen_boiler_controller_config.with_domestic_hot_water_preparation = True
    my_hydrogen_boiler_controller_config.set_heating_threshold_outside_temperature_in_celsius = (
    ...
    my_simple_heat_water_storage_config = simple_water_storage.SimpleHotWaterStorageConfig.get_scaled_hot_water_storage(
        max_thermal_power_in_watt_of_heating_system=concrete(my_hydrogen_boiler_config.maximal_thermal_power_in_watt),
    ...
    my_gas_meter = gas_meter.GasMeter(
        my_simulation_parameters=my_simulation_parameters,
        # The meter measures what the boiler burns -- green hydrogen here -- so the carrier is a
        # fact and not a second statement of the same thing.
        config=gas_meter.GasMeterConfig.preset_standard("GasMeter").resolve(
            SizingContext(energy_carrier=my_hydrogen_boiler_config.energy_carrier)
        ),
    )
