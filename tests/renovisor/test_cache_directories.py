...
new entry is written to the first writable one, a read-only seed is read but never written, and
the environment variable a container sets fills the parameter list. Unset, everything behaves
...
from hisim.caching.local import CacheEntryMetadata
from hisim.caching.locations import CacheLocations, CacheLocationsError
...
class TestAReadOnlySeed:
    """The bead's done-when: a read-only seed directory plus a writable one (hisim-epc.22).

    The seed is populated, then made read-only. Reads find its entries where they lie, a stale
    entry there is a miss and not an error, and every write lands in the writable directory
    whichever position the seed has in the list. The tests skip where the user is not bound by
    directory permissions (root in a container), because a read-only seed cannot be staged there.
    """

    @pytest.fixture
    def seed_and_volume(self, tmp_path: pathlib.Path) -> Iterator[Tuple[pathlib.Path, pathlib.Path]]:
        """A populated seed made read-only and an empty writable volume; permissions are restored after."""
        seed, volume = tmp_path / "seed", tmp_path / "volume"
        seed.mkdir()
        volume.mkdir()
        client = Given.client()
        with client.lookup(Given.COMPONENT_KEY, Given.KEY_MATERIAL, str(seed)).writing() as path:
            pathlib.Path(path).write_text(Given.PAYLOAD, encoding="utf-8")
        stale = seed / client.entry_filename(Given.COMPONENT_KEY, CacheEntryMetadata.hash_of(self.STALE_MATERIAL))
        stale.write_text("no metadata companion", encoding="utf-8")
        seed.chmod(stat.S_IRUSR | stat.S_IXUSR)
        if os.access(seed, os.W_OK):
            seed.chmod(stat.S_IRWXU)
            pytest.skip("this user is not bound by directory permissions, so a read-only seed cannot be staged")
        try:
            yield seed, volume
        finally:
            seed.chmod(stat.S_IRWXU)

    #: Key material of the stale entry: a file without the ``.meta`` companion the client demands.
    STALE_MATERIAL: str = '{"location": "Bonn"}2019-01-01###2020-01-01###3600'

    def test_an_entry_in_the_read_only_seed_is_read_from_there(
        self, seed_and_volume: Tuple[pathlib.Path, pathlib.Path]
    ) -> None:
        """A hit in the seed is returned at the seed's path, whatever comes first in the list."""
        seed, volume = seed_and_volume

        hit = Given.client().lookup_across(
            Given.COMPONENT_KEY, Given.KEY_MATERIAL, CacheLocations([str(volume), str(seed)])
        )

        assert hit.exists is True
        assert hit.path.startswith(str(seed))
        assert not any(volume.iterdir()), "reading the seed wrote nothing into the volume"

    def test_a_stale_entry_in_the_read_only_seed_is_a_miss_and_not_an_error(
        self, seed_and_volume: Tuple[pathlib.Path, pathlib.Path]
    ) -> None:
        """A file without its metadata companion is a miss; read-only, it cannot be discarded and stays."""
        seed, volume = seed_and_volume

        miss = Given.client().lookup_across(
            Given.COMPONENT_KEY, self.STALE_MATERIAL, CacheLocations([str(volume), str(seed)])
        )

        assert miss.exists is False
        assert miss.path.startswith(str(volume))

    def test_a_read_only_seed_listed_first_is_skipped_for_writes(
        self, seed_and_volume: Tuple[pathlib.Path, pathlib.Path]
    ) -> None:
        """The write directory is the first writable one, not the first one."""
        seed, volume = seed_and_volume
        locations = CacheLocations([str(seed), str(volume)])

        assert locations.write_directory() == str(volume)
        miss = Given.client().lookup_across(Given.COMPONENT_KEY, "never computed before", locations)
        assert miss.exists is False
        assert miss.path.startswith(str(volume))

    def test_a_missing_directory_under_the_read_only_seed_is_skipped(
        self, seed_and_volume: Tuple[pathlib.Path, pathlib.Path]
    ) -> None:
        """A listed directory that cannot be created is read past and not written to."""
        seed, volume = seed_and_volume
        locations = CacheLocations([str(seed / "absent"), str(volume)])

        assert locations.read("anything.cache") is None
        assert locations.write_directory() == str(volume)

    def test_nowhere_to_write_is_refused_by_name(self, seed_and_volume: Tuple[pathlib.Path, pathlib.Path]) -> None:
        """Only read-only directories: the error names them instead of a PermissionError inside a component."""
        seed, _ = seed_and_volume

        with pytest.raises(CacheLocationsError, match="absent"):
            CacheLocations([str(seed), str(seed / "absent")]).write_directory()
...
class TestTheEnvironmentVariable:
    ...
    def test_an_explicit_directory_outranks_the_variable(
        self, tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A caller that names a cache directory gets that one; the variable is not consulted."""
        explicit, seed = tmp_path / "explicit", tmp_path / "seed"
        monkeypatch.setenv(SimulationSetup.CACHE_DIRECTORIES_VARIABLE, str(seed))
        parameters = self._parameters(tmp_path, cache_directory=explicit)

        assert parameters.cache_directories == []
        assert parameters.cache_dir_path == str(explicit)
        assert parameters.cache_locations().directories == (str(explicit),)
    ...
    @staticmethod
    def _parameters(tmp_path: pathlib.Path, cache_directory: pathlib.Path | None = None) -> SimulationParameters:
