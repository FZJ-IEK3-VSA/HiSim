...
def _running_streak_counter(is_active_profile: list[int], seconds_per_timestep: int) -> list[int]:
...
def test_get_component_kpi_entries_heating_and_cooling_hours_match_active_time(
    is_active_profile: list[int],
) -> None:
    """Heating/cooling hours must equal the actual time the heat pump was on."""
    simpars = SimulationParameters.one_day_only(2017, 60)
    config = HeatPumpHplibConfig.get_default_generic_advanced_hp_lib()
    heatpump = HeatPumpHplib(config=config, my_simulation_parameters=simpars)
