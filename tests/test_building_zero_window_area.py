...
either way, so the kept U-value is the material's property, unused by the physics. A window
area *configured* for such a row is refused by name (``ZeroReferenceWindowAreaError``): it
would count in the transmission losses and vanish from every orientation.
...
floor element, which this guard does not cover (hisim-4g9.16) -- and stay pinned as
``raises:`` entries.
"""
...
class TestTheZeroWindowAreaGuard:
    ...
    def test_a_configured_window_area_is_refused_by_name(self) -> None:
        ...
        with pytest.raises(ZeroReferenceWindowAreaError, match="25.0 m2"):
