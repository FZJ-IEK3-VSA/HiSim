...
ordinary cost subjects: the thirteen envelope `ComponentType`s in the device data, the three
...
#: The thirteen asset classes that make up the envelope (README §3.2b). All are sized in m2 of
#: the respective element except EXTERIOR_DOOR and VENTILATION_SYSTEM, which are priced per unit.
ENVELOPE_CLASSES = [
    ComponentType.WALL_EXTERNAL_INSULATION,
    ComponentType.WALL_INTERNAL_INSULATION,
    ComponentType.ROOF_INSULATION_BETWEEN_JOISTS,
    ComponentType.ROOF_INSULATION_OVER_JOISTS,
    ComponentType.WARM_ROOF_INSULATION,
    ComponentType.TOP_CEILING_UPPER_INSULATION,
    ComponentType.BASEMENT_CEILING_BOTTOM_INSULATION,
    ComponentType.BASEMENT_WALLS_INTERNAL_INSULATION,
    ComponentType.GROUND_EXTERNAL_INSULATION,
    ComponentType.WINDOWS_TRIPLE_GLAZED,
...
class TestEnvelopeData:
    ...
    def test_entries_exist_for_both_countries_and_years(self, database, asset_class):
        ...
        for country, year in (("DE", 2026), ("DE", 2035), ("IE", 2026), ("IE", 2035)):
            ...
            assert entry.energy_related_cost_share.best_estimate == pytest.approx(1.0)
...
class TestEnvelopeTimeline:
    ...
    def test_residual_value_of_long_lived_measure(self, database):
        ...
        assert residual.best_estimate == pytest.approx(-investment.best_estimate / 2.0)
    ...
    def test_like_for_like_measure_is_charged_not_kept(self, database):
        ...
        assert categories[CostCategory.ANYWAY_COST_CREDIT].best_estimate < 0
    ...
    def test_kept_windows_without_measure_stay_kept(self, database):
        ...
        facts = ComponentCostFacts(asset_class=ComponentType.WINDOWS_TRIPLE_GLAZED, size=25.0, size_unit=Units.SQUARE_METER)
        evaluator = EconomicEvaluator(database, zero_rate_parameters())
        inputs = EvaluationInputs(
            simulation_year=2026,
            simulated_period_fraction=1.0,
    ...
    def test_coupled_cost_credit_via_energy_related_share(self, database):
        ...
        overlaid = database.with_overlays(
            {"devices_DE.WALL_EXTERNAL_INSULATION.energy_related_cost_share": {"min": 0.4, "best_estimate": 0.55, "max": 0.7}},
            "coupled_cost_test",
        )
        ...
        register = ExistingAssetRegister(
            assets=[
                ExistingAsset(
                    asset_class=ComponentType.WALL_EXTERNAL_INSULATION,  # the old facade "element"
                    size=100.0,
                    size_unit=Units.SQUARE_METER,
        ...
        assert credit.best_estimate == pytest.approx(-gross.best_estimate * (1.0 - 0.55))
...
class TestEnvelopeSubsidies:
    ...
    def _context(self, has_renovation_roadmap) -> SubsidyContext:
        ...
        undiluted. `has_renovation_roadmap` is passed through as the tri-state it is: True, False, or None for
        ...
        return SubsidyContext(
            ...
            building=SubsidyBuildingContext(
                ...
                has_renovation_roadmap=has_renovation_roadmap,
            ),
        )
    ...
    def test_base_rate_plus_isfp_bonus(self):
        ...
        decision = solve_cumulation(
            catalog, self._measure(), self._context(has_renovation_roadmap=True), 2026, self.DISCOUNT
        )
        ...
        total = sum(award.upfront_amount.best_estimate for award in decision.applied)
    ...
    def test_u_value_condition_rejects_poor_insulation(self):
        ...
        catalog = SubsidyCatalog.load("DE")
        decision = solve_cumulation(
            catalog, self._measure(u_value=0.35), self._context(has_renovation_roadmap=False), 2026, self.DISCOUNT
        )
    ...
    def test_unknown_isfp_is_undetermined_and_asked(self):
        ...
        catalog = SubsidyCatalog.load("DE")
        context = self._context(has_renovation_roadmap=None)
        ...
        assert "building.has_renovation_roadmap" in [question.entry.fieldname for question in questions]
