...
def _parse(module_name: str) -> ast.Module:
    """Parses one `hisim/economics/<module>.py` into a syntax tree.
    ...
    what the file says. The assertion on the path turns a renamed or moved module into an
    immediate, obvious failure instead of a silently empty import list that passes every check.
    """
    path = os.path.join(PACKAGE_DIRECTORY, f"{module_name}.py")
...
class TestPresentationImportsNoEngine:
    ...
    def test_only_the_allowed_surface_is_imported(self, module_name):
        ...
        violations = [
            ...
            if item.module not in ALLOWED_FOR_PRESENTATION
        ]
