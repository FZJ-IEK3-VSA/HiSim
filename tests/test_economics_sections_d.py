...
from hisim.economics.results import EvaluationMatrix, HeatCostNaming
...
class TestAnywayShareCaption:
    ...
    def test_a_run_that_credits_nothing_says_nothing(self, database):
        ...
        assert _anyway_share_caption(result) == ""
...
class TestScenarioRowsStateTheirAssumption:
    ...
    def test_every_row_names_the_assumption_with_both_values(self, report):
        ...
        assert "interest_rate 5.00% (central 3.00%)" in report
        ...
        assert "energy_price_escalation_rates.ELECTRICITY 0.00% (central 2.00%)" in report
    ...
    def test_an_inert_axis_footnotes_the_cause_read_off_the_timeline(self, report):
        """The all-electric run books no carbon price, so the CO2 axis moves nothing and says so."""
        assert (
            "<b>co2=high</b> — swing is exactly zero: no CO2-price flow is booked in this run"
            in report
        )
        assert "co2_price_exposure = 0" in report
        assert "electricity price entry declares" in report
