...
dependency and live in `test_economics_subsidies.py`, so that each stage of the review
...
def fixture_catalog() -> SubsidyCatalog:
    ...
    Duplicated from `test_economics_subsidies.py` rather than shared through a conftest:
...
def full_context(income: float = 35000.0) -> SubsidyContext:
    ...
    The same helper as in `test_economics_subsidies.py`, duplicated for the same reason as
...
class TestScenarioAnalysis:
    ...
    `test_economics_subsidies.py` as `TestScenarioDataOverlays`.
    """
    ...
    def test_cube_swings_and_spreads_match_the_report_derivation(self):
        ...
        base_value = per_scenario["central"].equivalent_annual_cost_in_euro.best_estimate
        expected_swings = {
            scenario_id: result.equivalent_annual_cost_in_euro.best_estimate - base_value
        ...
        values = [result.equivalent_annual_cost_in_euro.best_estimate for result in per_scenario.values()]
    ...
    def test_break_even_finds_crossing(self):
        ...
        def annual_cost_delta(interest_rate: float) -> float:
            ...
            return (
                capex_heavy.equivalent_annual_cost_in_euro.best_estimate
                - opex_heavy.equivalent_annual_cost_in_euro.best_estimate
            )
...
class TestDeletedDeadSurfaceInTheEngine:
    ...
    here rather than beside its sibling in `test_economics_subsidies.py`. It pins that a
