...
class TestTimeSeriesViews:
    ...
    def test_cumulative_discounted_cost_matches_manual_discounting(self, result):
        ...
        for slot, attribute in ((Slot.LOW, "minimum"), (Slot.AVERAGE, "average"), (Slot.HIGH, "maximum")):
    ...
    def test_cumulative_series_ends_at_the_reported_npv(self, result):
        ...
        assert series[Slot.AVERAGE][-1] == pytest.approx(npv.average)
            ...
            for entry in result.scoped_timeline().entries:
                if entry.year == year:
                    expected[entry.category] = (
                        expected.get(entry.category, 0.0) + entry.amount_in_euro.average
                    )
    ...
    def test_annual_matrix_row_sums_are_the_liquidity_view(self, result):
        ...
        for year, row in enumerate(matrix):
            assert sum(row.values()) == pytest.approx(
                result.annual_cost_series_nominal_in_euro[year].average, abs=1e-9
            )
    ...
    def test_fold_categories_sums_groups_and_rejects_gaps(self, result):
        ...
        assert sum(band.average for band in folded.values()) == pytest.approx(
            sum(band.average for band in result.npv_by_category.values())
        )
        expected_energy = sum(
            band.average
        ...
        assert folded["energy"].average == pytest.approx(expected_energy)
...
class TestDetailTable:
    ...
    def test_rows_and_subtotals_match_manual_aggregation(self, result):
        ...
        for detail_year in years:
            for row in detail_year.rows:
                ...
                assert row.nominal_in_euro.average == pytest.approx(expected.average)
                assert row.nominal_in_euro.minimum == pytest.approx(expected.minimum)
                assert row.discounted_average_in_euro == pytest.approx(
                    expected.average / ((1 + interest) ** row.year)
                )
            assert detail_year.nominal_total_in_euro.average == pytest.approx(
                sum(row.nominal_in_euro.average for row in detail_year.rows)
            )
            assert detail_year.discounted_total_average_in_euro == pytest.approx(
                detail_year.nominal_total_in_euro.average / ((1 + interest) ** detail_year.year)
            )
    ...
    def test_table_covers_the_whole_timeline(self, result):
        ...
        years = views.timeline_detail_rows(result)
        table_total = sum(
            row.nominal_in_euro.average for detail_year in years for row in detail_year.rows
        )
        timeline_total = sum(
            entry.amount_in_euro.average for entry in result.scoped_timeline().entries
        )
    ...
    def test_years_are_ordered_and_rows_sorted_by_amount(self, result):
        ...
        for detail_year in years:
            amounts = [row.nominal_in_euro.average for row in detail_year.rows]
...
class TestPivotsAndAnnuities:
    ...
    def test_payer_pivot_matches_manual_discounting(self, allocated):
        ...
        for payer, by_category in pivot.items():
            for category, band in by_category.items():
                ...
                assert band.average == pytest.approx(expected.average)
    ...
    def test_payer_pivot_reconciles_with_the_payer_npvs(self, allocated):
        ...
        pivot = views.payer_category_npv_pivot(allocated)
        for payer, by_category in pivot.items():
            row_sum = sum(band.average for band in by_category.values())
            assert row_sum == pytest.approx(allocated.npv_by_payer[payer].average)
    ...
    def test_payer_npv_total_is_the_zero_sum_reference(self, allocated):
        ...
        assert total.average == pytest.approx(expected.average)
    ...
    def test_equivalent_annual_costs_are_npv_times_annuity(self, result):
        ...
        for category, band in by_category.items():
            assert band.average == pytest.approx(result.npv_by_category[category].average * annuity)
...
class TestEnergyBills:
        ...
        for entry in result.scoped_timeline().entries:
            if entry.year == 1 and entry.subject in ("ELECTRICITY", "ELECTRICITY_FEED_IN"):
                expected_by_category[entry.category] = (
                    expected_by_category.get(entry.category, 0.0) + entry.amount_in_euro.average
                )
    ...
    def test_feed_in_is_shown_but_never_priced_into_the_unit_cost(self, result):
        ...
        assert bill.year_one_band_in_euro.average == pytest.approx(band_expected.average)
        assert bill.year_one_band_in_euro.average == pytest.approx(bill.total_excluding_feed_in_in_euro)
...
class TestYearZeroAndSubsidies:
        ...
        for subject, build_up in build_ups.items():
            if build_up.by_category_in_euro.get(CostCategory.INVESTMENT):
                ...
                assert build_up.by_category_in_euro[CostCategory.INVESTMENT] == pytest.approx(
                    allocated.component_breakdowns[subject].investment_gross_in_euro.average
                )
    ...
    def test_subsidy_share_matches_manual_ratio(self, result):
        ...
        for subject, share in shares.items():
            ...
            subsidy = min(breakdown.subsidies_nominal_in_euro.average, gross)
    ...
    def test_support_above_gross_is_clamped(self, result):
        ...
        assert share.subsidy_in_euro == pytest.approx(gross.average)
    ...
    def test_total_subsidies_received_sums_the_timeline_entries(self, result):
        ...
        assert total.average > 0  # a received-support figure is reported positive
    ...
    def test_total_subsidies_received_includes_support_without_an_award(self, result):
        ...
        total = views.total_subsidies_received(result)
        assert total is not None
        assert total.average > awards_only.average  # the fixture has a scheduled payout
    ...
    def test_investment_net_is_the_unclamped_band_difference(self, result):
        ...
        for subject, breakdown in result.component_breakdowns.items():
            ...
            assert nets[subject].average == pytest.approx(expected.average)
