...
def test_photovoltaic_sandia():
    ...
    mysim = sim.SimulationParameters.full_year(year=2021, seconds_per_timestep=seconds_per_timestep)
    ...
    assert pytest.approx(stsv.values[my_pvs.electricity_output_channel.global_index]) == 334.8800144821672
...
def test_photovoltaic_cec():
    ...
    # check pv electricity output [W] in timestep 655
    assert pytest.approx(stsv.values[my_pvs.electricity_output_channel.global_index]) == 340.552602382255

    # Check pv energy output channel [Wh] which should be the electricity
    # output in W times the timestep length in hours
    assert pytest.approx(stsv.values[my_pvs.electricity_energy_output_channel.global_index]) == 340.552602382255 * (
        seconds_per_timestep / 3600
    )
