...
def test_hds():
    ...
    # ===================================================================================================================
    SingletonSimRepository().set_entry(key=SingletonDictKeyEnum.MAXTHERMALBUILDINGDEMAND, entry=9000)
    ...
    timestep = 300
    water_mass_flow_of_hds_in_kg_per_second = SingletonSimRepository().get_entry(key=SingletonDictKeyEnum.WATERMASSFLOWRATEOFHEATINGDISTRIBUTIONSYSTEM)
    # Simulate
    ...
    for demand in theoretical_thermal_building_demands_in_watt:
        ...
        print("calc output " , calculated_water_output_temperature_in_celsius)
        log.information(
            "water output temperature after heat exchange with building "
            + str(water_output_temperature_in_celsius_from_simulation)
        )
        log.information(
            "thermal output delivered from hds " + str(stsv.values[5]) + "\n"
        )

        if stsv.values[theoretical_thermal_building_demand.global_index] > 0:
            ...
            elif calculated_water_output_temperature_in_celsius < stsv.values[residence_temperature_indoor_air.global_index]:
