...
def _build_building(floor_area_in_m2: float = 121.2) -> building.Building:
    ...
    # measure_execution_time is untyped, so the constructor call resolves to Any without this annotation.
    result: building.Building = building.Building(config=config, my_simulation_parameters=_simulation_parameters())
    return result
...
def _hds(
    ...
    hds_result: heat_distribution_system.HeatDistribution = heat_distribution_system.HeatDistribution(
