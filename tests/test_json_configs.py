...
SYSTEM_SETUP_DIR: Path = Path(__file__).resolve().parent.parent / "system_setups"
...
def discover_system_setups(base_dir: Path = SYSTEM_SETUP_DIR) -> list[tuple[str]]:
    ...
    ``pytest.mark.parametrize``. The default ``base_dir`` is ``SYSTEM_SETUP_DIR``,
    which is anchored to this module's file location
    (``Path(__file__).resolve().parent.parent / "system_setups"``) so discovery
    does not depend on the current working directory. Passing an explicit
    ``base_dir`` makes the filesystem collaborator injectable and the
    parametrization deterministic in isolation (e.g. for tests that need to
    pin which setups are exercised).
    """
    return [
        (str(path),)
...
SYSTEM_SETUPS: list[tuple[str]] = discover_system_setups(base_dir=SYSTEM_SETUP_DIR)
