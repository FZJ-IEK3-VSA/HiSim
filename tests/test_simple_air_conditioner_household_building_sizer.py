...
3.  That the recorded ``.energy_system.yaml`` twin declares the same four
...
# Path to the system setup module and its recorded energy-system twin, resolved
# relative to the repository root so the tests work regardless of the working directory.
...
_ENERGY_SYSTEM_YAML: Path = Path(__file__).resolve().parent.parent / "energy_systems" / (
    _SETUP_MODULE + ".energy_system.yaml"
)
...
# ==============================================================================
# 3. Recorded energy-system twin matches the Python setup
# ==============================================================================


@pytest.mark.base
def test_recorded_twin_structure() -> None:
    """The recorded twin declares 4 components and the one manual connection.

    Verifies that ``energy_systems/<setup>.energy_system.yaml`` is valid YAML, names the
    four expected component classes, and declares the manual
    SimpleAirConditioner.ThermalPowerDelivered -> Building.ThermalPowerDelivered
    connection. This replaced the same check against the setup's ``.scenario.json``
    twin, which retired with the v1 path on 2026-09-12; the twin is regenerated by the
    recorder and held current by the ``energy-system-freshness`` gate.
    """
    twin = yaml.safe_load(_ENERGY_SYSTEM_YAML.read_text(encoding="utf-8"))
    components = twin["components"]

    # Four components with the expected class names
    assert {entry["class"] for entry in components.values()} == {
        "hisim.components.building.building.Building",
    ...
    # The other input entries are bare component names, the twin's spelling of a default
    # connection; only this one names both an input port and the output feeding it.
    named = [
        feed
        for entry in components.values()
        for feed in entry.get("inputs", [])
        if isinstance(feed, dict)
    ]
    assert named == [
        {"input": "ThermalPowerDelivered", "from": "SimpleAirConditioner.ThermalPowerDelivered"}
    ]
