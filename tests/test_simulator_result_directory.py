...
@pytest.fixture(name="clean_path_provider", autouse=True)
def fixture_clean_path_provider():
    ...
    Autouse rather than requested per test: every test in this file needs it, and none of them has
...
def _simulator(directory: str, module_filename: str) -> Simulator:
    ...
    Args:
        directory: Stands in for a setup module's directory; `results/` is created underneath it.
        module_filename: Stands in for the setup module's name, which names the run's directory.

    Returns:
        The simulator, before any component or result directory is attached.
    """
    # Annotated rather than returned directly: `Simulator.__init__` carries no annotations, so
    # mypy infers `Any` and the tests' profile rejects returning it from a typed function.
...
class TestResultDirectorySelection:
    ...
    def test_a_second_run_does_not_inherit_the_first_runs_directory(self, tmp_path):
    ...
    def test_a_deliberately_configured_directory_is_still_honoured(self, tmp_path):
    ...
    def test_a_preset_result_directory_is_never_touched(self, tmp_path):
