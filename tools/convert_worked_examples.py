...
#: (a time-of-use band), so the check keys on the _min/_avg/_max triple convention instead.
BAND_LABEL_SUFFIXES = ("_min", "_avg", "_max", "_minimum", "_average", "_maximum")
BAND_LABEL_TOKENS = ("uncertainty", "_min_", "_avg_", "_max_")
...
def convert_workbook(path: str) -> Tuple[Example, str]:
    ...
    for row_index, cells in sections[INPUTS_MARKER]:
        ...
        if isinstance(raw, str) and re.match(r"^\s*-?[\d.]+\s*/\s*-?[\d.]+\s*/\s*-?[\d.]+\s*$", raw):
            raise ValidationError(f"row {row_index}: input {label!r} looks like a min/avg/max triple (rule 8).")
