    ...
    def register_method(func):
        ...
        def function_wrapper_for_node_storage(*args, **kwargs):
            ...
            for frame in curr_frame[1:]:
                ...
                if info in node_container.rank['Root'] and info not in node_container.wrapped_method_src[base_node]:
...
class MethodChart:
    ...
    def sum_time(self) -> None:
        ...
        if 'main' in self.rank['Root'][0]:
            ...
            summed_time = [sum(self.wrapped_method_timer[node]['time'] for node in self.wrapped_method_timer)]
    ...
    def set_depth(self,
        ...
        src_nodes = [node for node in rank if self.wrapped_method_src[node] == max_depth]
    ...
    def make_graphviz_chart(self,
        ...
        graph.set_node_defaults(color='black', style='filled', shape='box', fontname='Arial', fontsize='10')
        ...
        for rank in self.rank:
            for base_node in self.rank[rank]:
                ...
                if rank == 'Root':
                    my_node = pydot.Node(base_node, label=base_node+"\\n"+count_label+"\\n"+time_label, fillcolor=self.color_palette[base_node], style='filled, dashed')
                else:
                    my_node = pydot.Node(base_node, label=base_node+"\\n"+count_label+"\\n"+time_label, fillcolor=self.color_palette[base_node])
